...
public class UIFilmController extends UIElement
{
    ...
    private final Vector2i lastMouseHandler = new Vector2i();
    ...
    private void toggleMouseHandlerPointer(boolean disable)
    {
    ...
    private int getMouseHandlerMode()
    {
        return this.mouseMode % 6;
    }

    private void setMouseHandlerMode(int mode)
    {
        ...
        if (this.controlled != null)
        {
            ...
            int index = this.getMouseHandlerMode() - 1;
    ...
    private boolean isMouseHandlerLookMode()
    {
        return this.getMouseHandlerMode() == 0;
    }
    ...
    public void toggleControl()
    {
        ...
        this.setMouseHandlerMode(this.mouseMode);
        this.toggleMouseHandlerPointer(this.controlled != null);
    ...
    public void stopRecording()
    {
        ...
        this.setMouseHandlerMode(ClientNetwork.isIsBBSModOnServer() ? 0 : 1);
    }
    ...
    protected boolean subKeyPressed(UIContext context)
    {
        if (this.canControl())
        {
            ...
            else if (context.getKeyAction() == KeyAction.PRESSED && context.getKeyCode() >= GLFW.GLFW_KEY_1 && context.getKeyCode() <= GLFW.GLFW_KEY_6)
            {
                ...
                this.setMouseHandlerMode(context.getKeyCode() - GLFW.GLFW_KEY_1);
    ...
    private boolean canControlWithKeyboard(int utilKey)
    {
        ...
        return options.keyUp.getDefaultKey().getValue() == utilKey
            || options.keyDown.getDefaultKey().getValue() == utilKey
            || options.keyLeft.getDefaultKey().getValue() == utilKey
            || options.keyRight.getDefaultKey().getValue() == utilKey
            || options.keyShift.getDefaultKey().getValue() == utilKey
            || options.keySprint.getDefaultKey().getValue() == utilKey
            || options.keyJump.getDefaultKey().getValue() == utilKey;
    }
    ...
    public void pickRecording()
    {
        ...
        this.toggleMouseHandlerPointer(false);
    ...
    public void insertFrame()
    {
        ...
        if (Window.isCtrlPressed())
        {
            ...
            panel.onClose((event) -> this.toggleMouseHandlerPointer(this.controlled != null));
    ...
    private void updateControls()
    {
        ...
        if (!this.isMouseHandlerLookMode())
        {
            int index = this.getMouseHandlerMode() - 1;
    ...
    public void renderHUD(UIContext context, Area area)
    {
        ...
        int mode = this.getMouseHandlerMode();
        ...
        if (replay != null)
        {
            ...
            if (form != null)
            {
                ...
                context.batcher.clip(x, y - 10, 40, 40, area.w, area.h);
    ...
    private void renderPickingPreview(UIContext context, Area area)
    {
        ...
        /* setProjectionMatrix removed in MC 26.2 */ // RenderSystem.setProjectionMatrix(this.panel.lastProjection);

        /* Render the stencil */
        PoseStack worldStack = this.worldRenderContext.poseStack();

        worldStack.pushPose();
        worldStack.last().pose().identity();
        worldStack.last().normal().identity();
        PoseStackUtils.multiply(worldStack, this.panel.lastView);
        ...
        int target = previewProgram.getUniformLocation("Target");
    ...
    public void renderFrame(LevelRenderContext context)
    {
        ...
        int wx = (int) mouse.xpos();
        int wy = (int) mouse.ypos();

        if (this.canControl())
        {
            if (this.isMouseHandlerLookMode() && ClientNetwork.isIsBBSModOnServer())
            {
                float cursorDeltaX = (wx - this.lastMouseHandler.x) / 2F;
                float cursorDeltaY = (wy - this.lastMouseHandler.y) / 2F;
            ...
            else
            {
                ...
                float xx = (wy - this.lastMouseHandler.y) / sensitivity;
                float yy = (wx - this.lastMouseHandler.x) / sensitivity;
        ...
        this.lastMouseHandler.set(wx, wy);
    ...
    private void renderStencil(LevelRenderContext renderContext, UIContext context, boolean altPressed)
    {
        ...
        mchorse.bbs_mod.client.Framebuffer.window().beginWrite(true);
    }
