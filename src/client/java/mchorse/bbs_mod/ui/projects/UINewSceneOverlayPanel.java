...
/**
 * "New scene" dialog: a required scene name plus a background world picker
 * (blank world / any local singleplayer save in .minecraft/saves). The
 * picked world is stored on the scene and later used as the stage backdrop.
 */
public class UINewSceneOverlayPanel extends UIMessageBarOverlayPanel
{
    ...
    public UINewSceneOverlayPanel(BiConsumer<String, String> callback)
    {
        super(UIKeys.SCENES_CREATE, UIKeys.SCENES_NAME);

        this.callback = callback;

        this.name = new UITextbox(64, (s) -> this.updateConfirm());
        this.name.filename();
        this.name.placeholder(UIKeys.SCENES_NAME);

        var worldLabel = UI.label(UIKeys.SCENES_BACKGROUND, 20, Colors.LIGHTER_GRAY);
        worldLabel.h(20);
        ...
        this.addWorld(worldScroll, UIKeys.SCENES_BACKGROUND_BLANK.get(), "");
        ...
        this.content.add(UI.label(UIKeys.SCENES_NAME), this.name, worldLabel, worldScroll);
