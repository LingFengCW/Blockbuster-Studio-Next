...
extends Particle {
    ...
    public void extract(QuadParticleRenderState particleTypeRenderState, Camera camera, float partialTickTime) {
        ...
        if (this.roll != 0.0f) {
            rotation.rotateZ(Mth.lerp((float)partialTickTime, (float)this.oRoll, (float)this.roll));
        }
    ...
    protected void extractRotatedQuad(QuadParticleRenderState particleTypeRenderState, Camera camera, Quaternionf rotation, float partialTickTime) {
        ...
        float x = (float)(Mth.lerp((double)partialTickTime, (double)this.xo, (double)this.x) - pos.x());
        float y = (float)(Mth.lerp((double)partialTickTime, (double)this.yo, (double)this.y) - pos.y());
        float z = (float)(Mth.lerp((double)partialTickTime, (double)this.zo, (double)this.z) - pos.z());
    ...
    protected void extractRotatedQuad(QuadParticleRenderState particleTypeRenderState, Quaternionf rotation, float x, float y, float z, float partialTickTime) {
        particleTypeRenderState.add(this.getLayer(), x, y, z, rotation.x, rotation.y, rotation.z, rotation.w, this.getQuadSize(partialTickTime), this.getU0(), this.getU1(), this.getV0(), this.getV1(), ARGB.colorFromFloat((float)this.alpha, (float)this.rCol, (float)this.gCol, (float)this.bCol), this.getLightCoords(partialTickTime));
    }
    ...
    public record Layer(boolean translucent, Identifier textureAtlasLocation, RenderPipeline pipeline) {
        ...
        public static Layer bySprite(TextureAtlasSprite sprite) {
            ...
            if (sprite.atlasLocation().equals((Object)TextureAtlas.LOCATION_BLOCKS)) {
                return translucent ? TRANSLUCENT_TERRAIN : OPAQUE_TERRAIN;
            }
            if (sprite.atlasLocation().equals((Object)TextureAtlas.LOCATION_ITEMS)) {
