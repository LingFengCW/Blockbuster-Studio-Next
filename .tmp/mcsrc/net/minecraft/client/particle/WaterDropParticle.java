...
extends SingleQuadParticle {
    ...
    public void tick() {
        ...
        if ((offset = Math.max(this.level.getBlockState(pos = BlockPos.containing((double)this.x, (double)this.y, (double)this.z)).getCollisionShape((BlockGetter)this.level, pos).max(Direction.Axis.Y, this.x - (double)pos.getX(), this.z - (double)pos.getZ()), (double)this.level.getFluidState(pos).getHeight((BlockGetter)this.level, pos))) > 0.0 && this.y < (double)pos.getY() + offset) {
            this.remove();
        }
    }
