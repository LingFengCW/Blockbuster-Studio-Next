    ...
    extends NoRenderParticle {
        ...
        public void tick() {
            ...
            if (this.life % 2 == 0 && this.life / 2 < this.explosions.size()) {
                ...
                this.engine.createParticle((ParticleOptions)ColorParticleOption.create((ParticleType)ParticleTypes.FLASH, (int)color), this.x, this.y, this.z, 0.0, 0.0, 0.0);
            }
            ++this.life;
        ...
        private void createParticle(double x, double y, double z, double xa, double ya, double za, IntList rgbColors, IntList fadeColors, boolean trail, boolean twinkle) {
            SparkParticle sparkParticle = (SparkParticle)this.engine.createParticle((ParticleOptions)ParticleTypes.FIREWORK, x, y, z, xa, ya, za);
            sparkParticle.setTrail(trail);
            sparkParticle.setTwinkle(twinkle);
            sparkParticle.setAlpha(0.99f);
            sparkParticle.setColor((Integer)Util.getRandom((List)rgbColors, (RandomSource)this.random));
            if (!fadeColors.isEmpty()) {
                sparkParticle.setFadeColor((Integer)Util.getRandom((List)fadeColors, (RandomSource)this.random));
            }
        }
            ...
            for (int angleStep = 0; angleStep < 3; ++angleStep) {
                ...
                for (int c = 1; c < coords.length; ++c) {
                    ...
                    for (double subStep = 0.25; subStep <= 1.0; subStep += 0.25) {
                        double xa = Mth.lerp((double)subStep, (double)ox, (double)tx) * baseSpeed;
                        double ya = Mth.lerp((double)subStep, (double)oy, (double)ty) * baseSpeed;
