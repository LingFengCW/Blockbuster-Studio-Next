...
public abstract class Particle {
    ...
    public void move(double xa, double ya, double za) {
        ...
        if (this.hasPhysics && (xa != 0.0 || ya != 0.0 || za != 0.0) && xa * xa + ya * ya + za * za < MAXIMUM_COLLISION_VELOCITY_SQUARED) {
            Vec3 movement = Entity.collideBoundingBox((CollisionContext)CollisionContext.positionContext((double)this.y), (Vec3)new Vec3(xa, ya, za), (AABB)this.getBoundingBox(), (Level)this.level, List.of());
            xa = movement.x;
            ya = movement.y;
            za = movement.z;
        }
    ...
    protected int getLightCoords(float a) {
        ...
        if (this.level.hasChunkAt(pos)) {
            return LightCoordsUtil.getLightCoords((BlockAndLightGetter)this.level, (BlockPos)pos);
        }
        return 0xF00000;
    }
    ...
    public record LifetimeAlpha(float startAlpha, float endAlpha, float startAtNormalizedAge, float endAtNormalizedAge) {
        ...
        public float currentAlphaForAge(int age, int lifetime, float partialTickTime) {
            if (Mth.equal((float)this.startAlpha, (float)this.endAlpha)) {
                return this.startAlpha;
            }
            float timeNormalized = Mth.inverseLerp((float)(((float)age + partialTickTime) / (float)lifetime), (float)this.startAtNormalizedAge, (float)this.endAtNormalizedAge);
            return Mth.clampedLerp((float)timeNormalized, (float)this.startAlpha, (float)this.endAlpha);
        }
    }
}
