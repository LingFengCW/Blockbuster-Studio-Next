...
public record TrueTypeGlyphProviderDefinition(Identifier location, float size, float oversample, Shift shift, String skip) implements GlyphProviderDefinition
{
    ...
    public record Shift(float x, float y) {
        ...
        public static final Codec<Shift> CODEC = Codec.floatRange((float)-512.0f, (float)512.0f).listOf().comapFlatMap(input -> Util.fixedSize((List)input, (int)2).map(floats -> new Shift(((Float)floats.get(0)).floatValue(), ((Float)floats.get(1)).floatValue())), shift -> List.of(Float.valueOf(shift.x), Float.valueOf(shift.y)));
    }
}
