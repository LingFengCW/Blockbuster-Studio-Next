...
public class GuiGraphicsExtractor {
    ...
    public void textWithBackdrop(Font font, Component str, int textX, int textY, int textWidth, int textColor) {
        ...
        if (backgroundColor != 0) {
            int padding = 2;
            this.fill(textX - 2, textY - 2, textX + textWidth + 2, textY + font.lineHeight + 2, ARGB.multiply((int)backgroundColor, (int)textColor));
        }
    ...
    public void item(ItemStack itemStack, int x, int y) {
        this.item((LivingEntity)this.minecraft.player, this.minecraft.level, itemStack, x, y, 0);
    }

    public void item(ItemStack itemStack, int x, int y, int seed) {
        this.item((LivingEntity)this.minecraft.player, this.minecraft.level, itemStack, x, y, seed);
    }
    ...
    private void item(@Nullable LivingEntity owner, @Nullable Level level, ItemStack itemStack, int x, int y, int seed) {
        ...
        this.minecraft.getItemModelResolver().updateForTopItem(itemStackRenderState, itemStack, ItemDisplayContext.GUI, level, (ItemOwner)owner, seed);
        try {
    ...
    Consumer<Style> {
        ...
        public void accept(TextAlignment alignment, int anchorX, int y, ActiveTextCollector.Parameters parameters, FormattedCharSequence text) {
            ...
            GuiTextRenderState renderState = new GuiTextRenderState(this.this$0.minecraft.font, text, parameters.pose(), leftX, y, ARGB.white((float)parameters.opacity()), 0, true, needsFullStyleScan, parameters.scissor());
        ...
        public void acceptScrolling(Component message, int centerX, int left, int right, int top, int bottom, ActiveTextCollector.Parameters parameters) {
            int lineWidth = this.this$0.minecraft.font.width((FormattedText)message);
