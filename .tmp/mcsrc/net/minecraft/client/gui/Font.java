...
public class Font {
    ...
    private BakedGlyph getGlyph(int codepoint, Style style) {
        ...
        if (style.isObfuscated() && codepoint != 32) {
            int targetWidth = Mth.ceil((float)glyph.info().getAdvance(false));
    ...
    public PreparedText prepareText(String text, float x, float y, int originalColor, boolean drawShadow, int backgroundColor) {
        ...
        StringDecomposer.iterateFormatted((String)text, (Style)Style.EMPTY, (FormattedCharSink)output);
        return output;
    }
    ...
    public PreparedText prepareText(FormattedCharSequence text, float x, float y, int originalColor, boolean drawShadow, boolean includeEmpty, int backgroundColor) {
        ...
        text.accept((FormattedCharSink)builder);
        return builder;
    }

    public int width(String str) {
        return Mth.ceil((float)this.splitter.stringWidth(str));
    }

    public int width(FormattedText text) {
        return Mth.ceil((float)this.splitter.stringWidth(text));
    }
    ...
    FormattedCharSink {
        ...
        private int getShadowColor(Style style, int textColor) {
            ...
            if (shadow != null) {
                float textAlpha = ARGB.alphaFloat((int)textColor);
                float shadowAlpha = ARGB.alphaFloat((int)shadow);
                if (textAlpha != 1.0f) {
                    return ARGB.color((int)ARGB.as8BitChannel((float)(textAlpha * shadowAlpha)), (int)shadow);
                }
                return shadow;
            }
            if (this.drawShadow) {
        ...
        public @Nullable ScreenRectangle bounds() {
            ...
            int left = Mth.floor((float)this.left);
            int top = Mth.floor((float)this.top);
            int right = Mth.ceil((float)this.right);
            int bottom = Mth.ceil((float)this.bottom);
