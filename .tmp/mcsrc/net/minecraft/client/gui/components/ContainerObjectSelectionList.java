    ...
    implements ContainerEventHandler {
        ...
        public @Nullable ComponentPath nextFocusPath(FocusNavigationEvent navigationEvent) {
            if (navigationEvent instanceof FocusNavigationEvent.ArrowNavigation) {
                ...
                for (int i = index = Mth.clamp((int)(delta + this.children().indexOf(this.getFocused())), (int)0, (int)(this.children().size() - 1)); i >= 0 && i < this.children().size(); i += delta) {
        ...
        void updateNarration(NarrationElementOutput output) {
            ...
            if (result != null) {
                ...
                if (narratables.size() > 1) {
                    output.add(NarratedElementType.POSITION, (Component)Component.translatable((String)"narrator.position.object_list", (Object[])new Object[]{result.index() + 1, narratables.size()}));
                }
