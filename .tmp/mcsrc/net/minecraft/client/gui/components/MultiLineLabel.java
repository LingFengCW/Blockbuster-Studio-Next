        ...
        return new MultiLineLabel(){
            ...
            private List<TextAndWidth> getSplitMessage() {
                ...
                for (Component message : messages) {
                    splitMessage.addAll(font.splitIgnoringLanguage((FormattedText)message, maxWidth));
                }
                ...
                for (int i = 0; i < linesToAdd.size(); ++i) {
                    ...
                    if (i == linesToAdd.size() - 1 && actualMaxLines == maxLines && actualMaxLines != splitMessage.size()) {
                        ...
                        FormattedText withEllipsis = FormattedText.composite((FormattedText[])new FormattedText[]{clippedText, CommonComponents.ELLIPSIS.copy().withStyle(messages[messages.length - 1].getStyle())});
