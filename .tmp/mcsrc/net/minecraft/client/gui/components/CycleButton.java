...
implements ResettableOptionWidget {
    ...
    private T getCycledValue(int delta) {
        List<T> list = this.values.getSelectedList();
        return list.get(Mth.positiveModulo((int)(this.index + delta), (int)list.size()));
    }
    ...
    private MutableComponent createFullName(T newValue) {
        return CommonComponents.optionNameValue((Component)this.name, (Component)this.valueStringifier.apply(newValue));
    }
    ...
    public void updateWidgetNarration(NarrationElementOutput output) {
        ...
        if (this.active) {
            ...
            if (this.isFocused()) {
                output.add(NarratedElementType.USAGE, (Component)Component.translatable((String)"narration.cycle_button.usage.focused", (Object[])new Object[]{nextValueText}));
            } else {
                output.add(NarratedElementType.USAGE, (Component)Component.translatable((String)"narration.cycle_button.usage.hovered", (Object[])new Object[]{nextValueText}));
            }
        }
    }

    public MutableComponent createDefaultNarrationMessage() {
        return CycleButton.wrapDefaultNarrationMessage((Component)(this.displayState == DisplayState.VALUE ? this.createFullName(this.value) : this.getMessage()));
    }
    ...
    public static class Builder<T> {
        ...
        public CycleButton<T> create(int x, int y, int width, int height, Component name, OnValueChange<T> valueChangeListener) {
            ...
            Component initialTitle = this.displayState == DisplayState.VALUE ? valueText : CommonComponents.optionNameValue((Component)name, (Component)valueText);
