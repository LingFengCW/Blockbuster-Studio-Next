...
Renderable {
    ...
    protected void narrateListElementPosition(NarrationElementOutput output, TabButton widget) {
        ...
        if (this.tabs.size() > 1 && (index = this.tabButtons.indexOf((Object)widget)) != -1) {
            output.add(NarratedElementType.POSITION, (Component)Component.translatable((String)"narrator.position.tab", (Object[])new Object[]{index + 1, this.tabs.size()}));
        }
    }
    ...
    public boolean keyPressed(KeyEvent event) {
        ...
        if (event.hasControlDownWithQuirk() && (tabIndex = this.getNextTabIndex(event)) != -1) {
            this.selectTab(Mth.clamp((int)tabIndex, (int)0, (int)(this.tabs.size() - 1)), true);
            return true;
        }
        return false;
    }
