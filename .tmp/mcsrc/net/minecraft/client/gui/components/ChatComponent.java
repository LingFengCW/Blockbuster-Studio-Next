...
public class ChatComponent {
    ...
    private static final Component DELETED_CHAT_MESSAGE = Component.translatable((String)"chat.deleted_marker").withStyle(new ChatFormatting[]{ChatFormatting.GRAY, ChatFormatting.ITALIC});
    public static final int MESSAGE_BOTTOM_TO_MESSAGE_TOP = 8;
    public static final Identifier QUEUE_EXPAND_ID = Identifier.withDefaultNamespace((String)"internal/expand_chat_queue");
    private static final Style QUEUE_EXPAND_TEXT_STYLE = Style.EMPTY.withClickEvent((ClickEvent)new ClickEvent.Custom(QUEUE_EXPAND_ID, Optional.empty())).withHoverEvent((HoverEvent)new HoverEvent.ShowText((Component)Component.translatable((String)"chat.queue.tooltip")));
    public static final Identifier GO_TO_RESTRICTIONS_SCREEN = Identifier.withDefaultNamespace((String)"internal/go_to_restrictions_screen");
    private static final Component RESTRICTED_CHAT_MESSAGE = Component.translatable((String)"chat_screen.restricted").withStyle(Style.EMPTY.withColor(ChatFormatting.RED).withUnderlined(Boolean.valueOf(true)).withClickEvent((ClickEvent)new ClickEvent.Custom(GO_TO_RESTRICTIONS_SCREEN, Optional.empty())));
    private static final Component RESTRICTED_CHAT_MESSAGE_WITH_HOVER = ComponentUtils.mergeStyles((Component)RESTRICTED_CHAT_MESSAGE, (Style)Style.EMPTY.withHoverEvent((HoverEvent)new HoverEvent.ShowText((Component)Component.translatable((String)"chat_screen.restricted"))));
    ...
    private void extractRenderState(final ChatGraphicsAccess graphics, int screenHeight, int ticks, DisplayMode displayMode) {
        ...
        int maxWidth = Mth.ceil((float)((float)this.getWidth() / scale));
        final int chatBottom = Mth.floor((float)((float)(screenHeight - 40) / scale));
        ...
        int count = this.forEachLine(alphaCalculator, (line, lineIndex, alpha) -> {
            ...
            graphics.fill(-4, entryTop, maxWidth + 4 + 4, entryBottom, ARGB.black((float)(alpha * backgroundOpacity)));
        });
        int lineAboveMessagesY = chatBottom - (count + 1) * entryHeight;
        if (queueSize > 0L) {
            graphics.fill(-2, chatBottom, maxWidth + 4, chatBottom + messageHeight, ARGB.black((float)backgroundOpacity));
        }
        if (isRestricted) {
            graphics.fill(-2, lineAboveMessagesY, maxWidth + 4 + 4, lineAboveMessagesY + entryHeight, ARGB.black((float)backgroundOpacity));
        }
        ...
        if (queueSize > 0L) {
            ...
            MutableComponent queueMessage = Component.translatable((String)"chat.queue", (Object[])new Object[]{queueSize}).setStyle(QUEUE_EXPAND_TEXT_STYLE);
        ...
        if (total > 0 && isForeground) {
            ...
            if (virtualHeight != chatHeight) {
                ...
                graphics.fill(scrollBarStartX, -y, scrollBarStartX + 2, -y - height, ARGB.color((int)alpha2, (int)color));
                graphics.fill(scrollBarStartX + 2, -y, scrollBarStartX + 1, -y - height, ARGB.color((int)alpha2, (int)0xCCCCCC));
            }
        }
        profiler.pop();
    }
    ...
    private void logChatMessage(GuiMessage message) {
        ...
        String logTag = (String)Optionull.map((Object)message.tag(), GuiMessageTag::logTag);
        if (logTag != null) {
    ...
    private void addMessageToDisplayQueue(GuiMessage message) {
        int maxWidth = Mth.floor((double)((double)this.getWidth() / this.getScale()));
    ...
    private @Nullable DelayedMessageDeletion deleteMessageOrDelay(MessageSignature signature) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!signature.equals((Object)message.signature())) continue;
    ...
    public void addRecentChat(String message) {
        if (!message.equals(this.recentChat.peekLast())) {
            ...
            this.recentChat.addLast((Object)message);
        }
    ...
    public static int getWidth(double pct) {
        int max = 320;
        int min = 40;
        return Mth.floor((double)(pct * 280.0 + 40.0));
    }

    public static int getHeight(double pct) {
        int max = 180;
        int min = 20;
        return Mth.floor((double)(pct * 160.0 + 20.0));
    }
    ...
    Consumer<Style> {
        ...
        public void handleTag(int x0, int y0, int x1, int y1, float opacity, GuiMessageTag tag) {
            int indicatorColor = ARGB.color((float)opacity, (int)tag.indicatorColor());
        ...
        private void showTooltip(GuiMessageTag tag) {
            if (tag.text() != null) {
                this.graphics.setTooltipForNextFrame(this.font, this.font.split((FormattedText)tag.text(), 210), this.globalMouseX, this.globalMouseY);
            }
        }
    }
