...
public abstract class AbstractDebugChart {
    ...
    protected int getSampleColor(double sample, double min, int minColor, double mid, int midColor, double max, int maxColor) {
        if ((sample = Mth.clamp((double)sample, (double)min, (double)max)) < mid) {
            return ARGB.srgbLerp((float)((float)((sample - min) / (mid - min))), (int)minColor, (int)midColor);
        }
        return ARGB.srgbLerp((float)((float)((sample - mid) / (max - mid))), (int)midColor, (int)maxColor);
    }
}
