...
extends AbstractButton {
    private static final Identifier CHECKBOX_SELECTED_HIGHLIGHTED_SPRITE = Identifier.withDefaultNamespace((String)"widget/checkbox_selected_highlighted");
    private static final Identifier CHECKBOX_SELECTED_SPRITE = Identifier.withDefaultNamespace((String)"widget/checkbox_selected");
    private static final Identifier CHECKBOX_HIGHLIGHTED_SPRITE = Identifier.withDefaultNamespace((String)"widget/checkbox_highlighted");
    private static final Identifier CHECKBOX_SPRITE = Identifier.withDefaultNamespace((String)"widget/checkbox");
    ...
    private static int getDefaultWidth(Component message, Font font) {
        return Checkbox.getBoxSize(font) + 4 + font.width((FormattedText)message);
    }
    ...
    public void updateWidgetNarration(NarrationElementOutput output) {
        ...
        if (this.active) {
            if (this.isFocused()) {
                output.add(NarratedElementType.USAGE, (Component)Component.translatable((String)(this.selected ? "narration.checkbox.usage.focused.uncheck" : "narration.checkbox.usage.focused.check")));
            } else {
                output.add(NarratedElementType.USAGE, (Component)Component.translatable((String)(this.selected ? "narration.checkbox.usage.hovered.uncheck" : "narration.checkbox.usage.hovered.check")));
            }
        }
    }
    ...
    public void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, this.getX(), this.getY(), boxSize, boxSize, ARGB.white((float)this.alpha));
