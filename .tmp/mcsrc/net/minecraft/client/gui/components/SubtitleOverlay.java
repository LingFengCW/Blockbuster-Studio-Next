...
implements SoundEventListener {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics) {
        ...
        for (Subtitle subtitle : this.audibleSubtitles) {
            ...
            int brightness = Mth.floor((float)Mth.clampedLerp((float)((float)(Util.getMillis() - closestRecentLocation.time) / (float)(3000.0 * displayTimeMultiplier)), (float)255.0f, (float)75.0f));
            ...
            int textColor = ARGB.color((int)255, (int)brightness, (int)brightness, (int)brightness);
            if (!inView) {
                if (rightness > 0.0) {
    ...
    public void onPlaySound(SoundInstance sound, WeighedSoundEvents soundEvent, float range) {
        ...
        if (!this.subtitles.isEmpty()) {
            for (Subtitle subtitle : this.subtitles) {
                if (!subtitle.getText().equals((Object)text)) continue;
    ...
    private static class Subtitle {
        ...
        public void refresh(Vec3 location) {
            this.playedAt.removeIf(soundPlayedAt -> location.equals((Object)soundPlayedAt.location()));
            this.playedAt.add(new SoundPlayedAt(location, Util.getMillis()));
        }
        ...
        public boolean isAudibleFrom(Vec3 camera) {
            ...
            return camera.closerThan((Position)closest.location, (double)this.range);
        }
