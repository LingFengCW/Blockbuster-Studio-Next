...
implements Toast {
    private static final Identifier BACKGROUND_SPRITE = Identifier.withDefaultNamespace((String)"toast/tutorial");
    ...
    public TutorialToast(Font font, Icons icon, Component title, @Nullable Component message, boolean progressable, int timeToDisplayMs) {
        ...
        this.lines.addAll(font.split((FormattedText)title.copy().withColor(-11534256), 126));
        if (message != null) {
            this.lines.addAll(font.split((FormattedText)message, 126));
        }
    ...
    public void update(ToastManager manager, long fullyVisibleForMs) {
        if (this.timeToDisplayMs > 0) {
            ...
            this.smoothedProgress = Mth.clampedLerp((float)((float)(fullyVisibleForMs - this.lastSmoothingTime) / 100.0f), (float)this.smoothedProgress, (float)this.progress);
            this.lastSmoothingTime = fullyVisibleForMs;
        }
    }
    ...
    public static enum Icons {
        MOVEMENT_KEYS(Identifier.withDefaultNamespace((String)"toast/movement_keys")),
        MOUSE(Identifier.withDefaultNamespace((String)"toast/mouse")),
        TREE(Identifier.withDefaultNamespace((String)"toast/tree")),
        RECIPE_BOOK(Identifier.withDefaultNamespace((String)"toast/recipe_book")),
        WOODEN_PLANKS(Identifier.withDefaultNamespace((String)"toast/wooden_planks")),
        SOCIAL_INTERACTIONS(Identifier.withDefaultNamespace((String)"toast/social_interactions")),
        RIGHT_CLICK(Identifier.withDefaultNamespace((String)"toast/right_click"));
