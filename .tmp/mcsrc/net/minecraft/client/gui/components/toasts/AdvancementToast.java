...
implements Toast {
    private static final Identifier BACKGROUND_SPRITE = Identifier.withDefaultNamespace((String)"toast/advancement");
    ...
    private boolean isChallengeAdvancement() {
        Optional displayInfo = this.advancement.value().display();
        return displayInfo.isPresent() && ((DisplayInfo)displayInfo.get()).getType().equals((Object)AdvancementType.CHALLENGE);
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, Font font, long fullyVisibleForMs) {
        ...
        List<FormattedCharSequence> lines = font.split((FormattedText)display.getTitle(), 125);
        ...
        if (lines.size() == 1) {
            ...
            if (fullyVisibleForMs < 1500L) {
                int alpha = Mth.floor((float)(Mth.clamp((float)((float)(1500L - fullyVisibleForMs) / 300.0f), (float)0.0f, (float)1.0f) * 255.0f));
                graphics.text(font, display.getType().getDisplayName(), 30, 11, ARGB.color((int)alpha, (int)titleColor), false);
            } else {
                int alpha = Mth.floor((float)(Mth.clamp((float)((float)(fullyVisibleForMs - 1500L) / 300.0f), (float)0.0f, (float)1.0f) * 252.0f));
                int y = this.height() / 2 - lines.size() * font.lineHeight / 2;
                for (FormattedCharSequence line : lines) {
                    graphics.text(font, line, 30, y, ARGB.white((int)alpha), false);
                    y += font.lineHeight;
                }
            }
        }
