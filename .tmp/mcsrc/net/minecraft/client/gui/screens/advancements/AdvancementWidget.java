...
public class AdvancementWidget {
    private static final Identifier TITLE_BOX_SPRITE = Identifier.withDefaultNamespace((String)"advancements/title_box");
    ...
    public AdvancementWidget(AdvancementTab tab, Minecraft minecraft, AdvancementNode advancementNode, DisplayInfo display) {
        ...
        this.titleLines = minecraft.font.split((FormattedText)display.getTitle(), 163);
        this.x = Mth.floor((float)(display.getX() * 28.0f));
        this.y = Mth.floor((float)(display.getY() * 27.0f));
        ...
        this.description = Language.getInstance().getVisualOrder(this.findOptimalLines(ComponentUtils.mergeStyles((Component)display.getDescription(), (Style)Style.EMPTY.withColor(display.getType().getChatColor())), longestDescLine));
    ...
    private int getMaxProgressWidth() {
        ...
        int spacing = 8;
        MutableComponent fakeMaxProgress = Component.translatable((String)"advancements.progress", (Object[])new Object[]{maxCriteraRequired, maxCriteraRequired});
    ...
    public void extractHover(GuiGraphicsExtractor graphics, int xo, int yo, float fade, int screenxo, int screenyo) {
        ...
        float amount = this.progress == null ? 0.0f : this.progress.getPercent();
        int firstHalfWidth = Mth.floor((float)(amount * (float)this.width));
        if (amount >= 1.0f) {
