...
extends Screen {
    private static final Identifier VIGNETTE_LOCATION = Identifier.withDefaultNamespace((String)"textures/misc/credits_vignette.png");
    ...
    private static final Identifier END_POEM_LOCATION = Identifier.withDefaultNamespace((String)"texts/end.txt");
    private static final Identifier CREDITS_LOCATION = Identifier.withDefaultNamespace((String)"texts/credits.json");
    private static final Identifier POSTCREDITS_LOCATION = Identifier.withDefaultNamespace((String)"texts/postcredits.txt");
    ...
    public Component getNarrationMessage() {
        return CommonComponents.joinForNarration((Component[])((Component[])this.narratorComponents.toArray(Component[]::new)));
    }
    ...
    private void addPoemFile(Reader inputReader) throws IOException {
        ...
        RandomSource random = RandomSource.createThreadLocalInstance((long)8124371L);
    ...
    private void addCreditsFile(Reader inputReader) {
        JsonArray root = GsonHelper.parseArray((Reader)inputReader);
        for (JsonElement sectionElement : root) {
            ...
            this.addCreditsLine((Component)Component.literal((String)sectionName).withStyle(ChatFormatting.YELLOW), true, true);
            this.addCreditsLine(SECTION_HEADING, true, false);
            this.addEmptyLine();
            this.addEmptyLine();
            ...
            for (JsonElement disciplineElement : disciplines) {
                ...
                if (StringUtils.isNotEmpty((CharSequence)disciplineName)) {
                    this.addCreditsLine((Component)Component.literal((String)disciplineName).withStyle(ChatFormatting.YELLOW), true, true);
                    this.addEmptyLine();
                    this.addEmptyLine();
                }
                ...
                for (JsonElement titleElement : titles) {
                    ...
                    this.addCreditsLine((Component)Component.literal((String)titleName).withStyle(ChatFormatting.GRAY), false, true);
                    for (JsonElement nameElement : names) {
                        String name = nameElement.getAsString();
                        this.addCreditsLine((Component)Component.literal((String)NAME_PREFIX).append(name).withStyle(ChatFormatting.WHITE), false, true);
                    }
                    this.addEmptyLine();
                    this.addEmptyLine();
                }
    ...
    private void addPoemLines(String line) {
        MutableComponent component = Component.literal((String)line);
        this.lines.addAll(this.minecraft.font.split((FormattedText)component, 256));
        this.narratorComponents.add((Component)component);
    }
