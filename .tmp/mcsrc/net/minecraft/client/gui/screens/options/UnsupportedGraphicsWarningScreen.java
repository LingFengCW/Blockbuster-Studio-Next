...
extends Screen {
    ...
    protected UnsupportedGraphicsWarningScreen(Component title, List<Component> message, ImmutableList<ButtonOption> buttonOptions) {
        ...
        this.narrationMessage = CommonComponents.joinForNarration((Component[])new Component[]{title, ComponentUtils.formatList(message, (Component)CommonComponents.EMPTY)});
    ...
    public void init() {
        for (ButtonOption buttonOption : this.buttonOptions) {
            this.buttonWidth = Math.max(this.buttonWidth, 20 + this.font.width((FormattedText)buttonOption.message) + 20);
        }
