...
extends Screen {
    private static final Component TITLE = Component.translatable((String)"restrictions_screen.title");
    ...
    protected void init() {
        ...
        this.chatAbilities.restrictions().forEach(restriction -> {
            body.addChild(FocusableTextWidget.builder(ComponentUtils.mergeStyles((Component)restriction.display(), (Style)Style.EMPTY.withBold(Boolean.valueOf(true)).withColor(ChatFormatting.RED)), this.font).maxWidth(250).alwaysShowBorder(false).backgroundFill(FocusableTextWidget.BackgroundFill.ON_FOCUS).build().setCentered(true));
    ...
    private static Component createPermissionStatus(ChatAbilities abilities, Permission permission, String permissionId) {
        ...
        if (isAllowed) {
            return Component.translatable((String)(prefix + ".allowed")).withStyle(ChatFormatting.GREEN);
        }
        return Component.translatable((String)(prefix + ".denied")).withStyle(ChatFormatting.RED);
    }
