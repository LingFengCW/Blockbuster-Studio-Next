...
extends AbstractContainerScreen<EnchantmentMenu> {
    ...
    private static final Identifier ENCHANTMENT_SLOT_DISABLED_SPRITE = Identifier.withDefaultNamespace((String)"container/enchanting_table/enchantment_slot_disabled");
    private static final Identifier ENCHANTMENT_SLOT_HIGHLIGHTED_SPRITE = Identifier.withDefaultNamespace((String)"container/enchanting_table/enchantment_slot_highlighted");
    private static final Identifier ENCHANTMENT_SLOT_SPRITE = Identifier.withDefaultNamespace((String)"container/enchanting_table/enchantment_slot");
    private static final Identifier ENCHANTING_TABLE_LOCATION = Identifier.withDefaultNamespace((String)"textures/gui/container/enchanting_table.png");
    private static final Identifier ENCHANTING_BOOK_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/enchantment/enchanting_table_book.png");
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (!(xx >= 0.0) || !(yy >= 0.0) || !(xx < 108.0) || !(yy < 19.0) || !((EnchantmentMenu)this.menu).clickMenuButton((Player)this.minecraft.player, i)) continue;
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (!(goldCount >= i + 1 && this.minecraft.player.experienceLevel >= cost || this.minecraft.player.hasInfiniteMaterials())) {
                ...
                graphics.textWithWordWrap(this.font, message, leftPosText, yo + 16 + 19 * i, textWidth, ARGB.opaque((int)((col & 0xFEFEFE) >> 1)), false);
                col = -12550384;
            } else {
                int xx = mouseX - (xo + 60);
    ...
    private void extractBook(GuiGraphicsExtractor graphics, int left, int top) {
        ...
        float open = Mth.lerp((float)a, (float)this.oOpen, (float)this.open);
        float flip = Mth.lerp((float)a, (float)this.oFlip, (float)this.flip);
        int x0 = left + 14;
        int y0 = top + 14;
        int x1 = x0 + 38;
        int y1 = y0 + 31;
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float ignored) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            texts.add(Component.translatable((String)"container.enchant.clue", (Object[])new Object[]{Enchantment.getFullname((Holder)((Holder)enchant.get()), (int)enchantLevel)}).withStyle(ChatFormatting.WHITE));
            if (!infiniteMaterials) {
                texts.add(CommonComponents.EMPTY);
                if (this.minecraft.player.experienceLevel < minLevel) {
                    texts.add(Component.translatable((String)"container.enchant.level.requirement", (Object[])new Object[]{((EnchantmentMenu)this.menu).costs[i]}).withStyle(ChatFormatting.RED));
                } else {
                    MutableComponent lapisCost = cost == 1 ? Component.translatable((String)"container.enchant.lapis.one") : Component.translatable((String)"container.enchant.lapis.many", (Object[])new Object[]{cost});
                    texts.add(lapisCost.withStyle(gold >= cost ? ChatFormatting.GRAY : ChatFormatting.RED));
                    MutableComponent levelCost = cost == 1 ? Component.translatable((String)"container.enchant.level.one") : Component.translatable((String)"container.enchant.level.many", (Object[])new Object[]{cost});
    ...
    public void tickBook() {
        ...
        if (!ItemStack.matches((ItemStack)current, (ItemStack)this.last)) {
            this.last = current;
            do {
        ...
        float max = 0.2f;
        diff = Mth.clamp((float)diff, (float)-0.2f, (float)0.2f);
