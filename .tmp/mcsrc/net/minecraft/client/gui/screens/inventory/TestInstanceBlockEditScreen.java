...
extends Screen {
    private static final Component ID_LABEL = Component.translatable((String)"test_instance_block.test_id");
    private static final Component SIZE_LABEL = Component.translatable((String)"test_instance_block.size");
    private static final Component INCLUDE_ENTITIES_LABEL = Component.translatable((String)"test_instance_block.entities");
    private static final Component ROTATION_LABEL = Component.translatable((String)"test_instance_block.rotation");
    ...
    public TestInstanceBlockEditScreen(TestInstanceBlockEntity blockEntity) {
        super((Component)blockEntity.getBlockState().getBlock().getName());
    ...
    protected void init() {
        ...
        this.idEdit = new EditBox(this.font, leftEdge, 40, 316, 20, (Component)Component.translatable((String)"test_instance_block.test_id"));
        this.idEdit.setMaxLength(128);
        Optional test = this.blockEntity.test();
        if (test.isPresent()) {
        ...
        this.infoWidget = new FittingMultiLineTextWidget(leftEdge, 70, 316, 8 * this.font.lineHeight, (Component)Component.literal((String)""), this.font);
        this.addRenderableWidget(this.infoWidget);
        Vec3i size = this.blockEntity.getSize();
        int index = 0;
        this.sizeXEdit = new EditBox(this.font, this.widgetX(index++, 5), 160, TestInstanceBlockEditScreen.widgetSize(5), 20, (Component)Component.translatable((String)"structure_block.size.x"));
        this.sizeXEdit.setMaxLength(15);
        this.addRenderableWidget(this.sizeXEdit);
        this.sizeYEdit = new EditBox(this.font, this.widgetX(index++, 5), 160, TestInstanceBlockEditScreen.widgetSize(5), 20, (Component)Component.translatable((String)"structure_block.size.y"));
        this.sizeYEdit.setMaxLength(15);
        this.addRenderableWidget(this.sizeYEdit);
        this.sizeZEdit = new EditBox(this.font, this.widgetX(index++, 5), 160, TestInstanceBlockEditScreen.widgetSize(5), 20, (Component)Component.translatable((String)"structure_block.size.z"));
        ...
        index = 0;
        this.addRenderableWidget(Button.builder((Component)Component.translatable((String)"test_instance.action.reset"), button -> {
        ...
        this.saveButton = this.addRenderableWidget(Button.builder((Component)Component.translatable((String)"test_instance.action.save"), button -> {
            this.sendToServer(ServerboundTestInstanceBlockActionPacket.Action.SAVE);
            this.minecraft.gui.setScreen(null);
        }).bounds(this.widgetX(index++, actionButtonCount), 185, buttonSize, 20).build());
        if (includeExport) {
            this.exportButton = this.addRenderableWidget(Button.builder((Component)Component.literal((String)"Export Structure"), button -> {
                this.sendToServer(ServerboundTestInstanceBlockActionPacket.Action.EXPORT);
                this.minecraft.gui.setScreen(null);
            }).bounds(this.widgetX(index++, actionButtonCount), 185, buttonSize, 20).build());
        }
        this.addRenderableWidget(Button.builder((Component)Component.translatable((String)"test_instance.action.run"), button -> {
    ...
    private void updateSaveState() {
        boolean allowSaving;
        this.saveButton.active = allowSaving = this.rotationButton.getValue() == Rotation.NONE && Identifier.tryParse((String)this.idEdit.getValue()) != null;
    ...
    private static Component rotationDisplay(Rotation rotation) {
        return Component.literal((String)(switch (rotation) {
    ...
    private void updateTestInfo(boolean isInit) {
        ...
        if (!valid) {
            this.infoWidget.setMessage((Component)Component.translatable((String)"test_instance.description.invalid_id").withStyle(ChatFormatting.RED));
        }
    ...
    private boolean sendToServer(ServerboundTestInstanceBlockActionPacket.Action action) {
        Optional<Identifier> id = Optional.ofNullable(Identifier.tryParse((String)this.idEdit.getValue()));
        Optional<ResourceKey> key = id.map(i -> ResourceKey.create((ResourceKey)Registries.TEST_INSTANCE, (Identifier)i));
        ...
        this.minecraft.getConnection().send((Packet<?>)new ServerboundTestInstanceBlockActionPacket(this.blockEntity.getBlockPos(), action, key, size, this.rotationButton.getValue(), ignoreEntities));
        return id.isPresent();
    }

    public void setStatus(Component status, Optional<Vec3i> size) {
        MutableComponent description = Component.empty();
        this.blockEntity.errorMessage().ifPresent(error -> description.append((Component)Component.translatable((String)"test_instance.description.failed", (Object[])new Object[]{Component.empty().withStyle(ChatFormatting.RED).append(error)})).append("\n\n"));
        description.append(status);
        this.infoWidget.setMessage((Component)description);
    ...
    private static int parseSize(String value) {
        try {
            return Mth.clamp((int)Integer.parseInt(value), (int)1, (int)48);
        }
