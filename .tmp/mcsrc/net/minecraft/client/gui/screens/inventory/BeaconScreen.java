...
extends AbstractContainerScreen<BeaconMenu> {
    private static final Identifier BEACON_LOCATION = Identifier.withDefaultNamespace((String)"textures/gui/container/beacon.png");
    private static final Identifier BUTTON_DISABLED_SPRITE = Identifier.withDefaultNamespace((String)"container/beacon/button_disabled");
    private static final Identifier BUTTON_SELECTED_SPRITE = Identifier.withDefaultNamespace((String)"container/beacon/button_selected");
    private static final Identifier BUTTON_HIGHLIGHTED_SPRITE = Identifier.withDefaultNamespace((String)"container/beacon/button_highlighted");
    private static final Identifier BUTTON_SPRITE = Identifier.withDefaultNamespace((String)"container/beacon/button");
    private static final Identifier CONFIRM_SPRITE = Identifier.withDefaultNamespace((String)"container/beacon/confirm");
    private static final Identifier CANCEL_SPRITE = Identifier.withDefaultNamespace((String)"container/beacon/cancel");
    private static final Component PRIMARY_EFFECT_LABEL = Component.translatable((String)"block.minecraft.beacon.primary");
    private static final Component SECONDARY_EFFECT_LABEL = Component.translatable((String)"block.minecraft.beacon.secondary");
    ...
    protected void init() {
        ...
        for (tier = 0; tier <= 2; ++tier) {
            count = ((List)BeaconBlockEntity.BEACON_EFFECTS.get(tier)).size();
            totalWidth = count * 22 + (count - 1) * 2;
            for (c = 0; c < count; ++c) {
                effect = (Holder)((List)BeaconBlockEntity.BEACON_EFFECTS.get(tier)).get(c);
                beaconPowerButton = new BeaconPowerButton(this, this.leftPos + 76 + c * 24 - totalWidth / 2, this.topPos + 22 + tier * 25, (Holder<MobEffect>)effect, true, tier);
                beaconPowerButton.active = false;
                this.addBeaconButton(beaconPowerButton);
            }
        }
        tier = 3;
        count = ((List)BeaconBlockEntity.BEACON_EFFECTS.get(3)).size() + 1;
        totalWidth = count * 22 + (count - 1) * 2;
        for (c = 0; c < count - 1; ++c) {
            effect = (Holder)((List)BeaconBlockEntity.BEACON_EFFECTS.get(3)).get(c);
            beaconPowerButton = new BeaconPowerButton(this, this.leftPos + 167 + c * 24 - totalWidth / 2, this.topPos + 47, (Holder<MobEffect>)effect, false, 3);
            beaconPowerButton.active = false;
            this.addBeaconButton(beaconPowerButton);
        }
        Holder dummyEffect = (Holder)((List)BeaconBlockEntity.BEACON_EFFECTS.get(0)).get(0);
        BeaconUpgradePowerButton beaconPowerButton2 = new BeaconUpgradePowerButton(this, this.leftPos + 167 + (count - 1) * 24 - totalWidth / 2, this.topPos + 47, (Holder<MobEffect>)dummyEffect);
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        graphics.item(new ItemStack((ItemLike)Items.NETHERITE_INGOT), xo + 20, yo + 109);
        graphics.item(new ItemStack((ItemLike)Items.EMERALD), xo + 41, yo + 109);
        graphics.item(new ItemStack((ItemLike)Items.DIAMOND), xo + 41 + 22, yo + 109);
        graphics.item(new ItemStack((ItemLike)Items.GOLD_INGOT), xo + 42 + 44, yo + 109);
        graphics.item(new ItemStack((ItemLike)Items.IRON_INGOT), xo + 42 + 66, yo + 109);
    }
    ...
    extends BeaconScreenButton {
        ...
        protected void setEffect(Holder<MobEffect> effect) {
            ...
            this.setTooltip(Tooltip.create((Component)this.createEffectDescription(effect), null));
        }

        protected MutableComponent createEffectDescription(Holder<MobEffect> effect) {
            return Component.translatable((String)((MobEffect)effect.value()).getDescriptionId());
        }
    ...
    extends BeaconPowerButton {
        ...
        @Override
        protected MutableComponent createEffectDescription(Holder<MobEffect> effect) {
            return Component.translatable((String)((MobEffect)effect.value()).getDescriptionId()).append(" II");
        }
    ...
    extends BeaconSpriteScreenButton {
        ...
        @Override
        public void onPress(InputWithModifiers input) {
            this.this$0.minecraft.getConnection().send((Packet<?>)new ServerboundSetBeaconPacket(Optional.ofNullable(this.this$0.primary), Optional.ofNullable(this.this$0.secondary)));
