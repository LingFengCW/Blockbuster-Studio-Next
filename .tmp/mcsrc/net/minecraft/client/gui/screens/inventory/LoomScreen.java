...
extends AbstractContainerScreen<LoomMenu> {
    private static final Identifier BANNER_SLOT_SPRITE = Identifier.withDefaultNamespace((String)"container/slot/banner");
    private static final Identifier DYE_SLOT_SPRITE = Identifier.withDefaultNamespace((String)"container/slot/dye");
    private static final Identifier PATTERN_SLOT_SPRITE = Identifier.withDefaultNamespace((String)"container/slot/banner_pattern");
    private static final Identifier SCROLLER_SPRITE = Identifier.withDefaultNamespace((String)"container/loom/scroller");
    private static final Identifier SCROLLER_DISABLED_SPRITE = Identifier.withDefaultNamespace((String)"container/loom/scroller_disabled");
    private static final Identifier PATTERN_SELECTED_SPRITE = Identifier.withDefaultNamespace((String)"container/loom/pattern_selected");
    private static final Identifier PATTERN_HIGHLIGHTED_SPRITE = Identifier.withDefaultNamespace((String)"container/loom/pattern_highlighted");
    private static final Identifier PATTERN_SPRITE = Identifier.withDefaultNamespace((String)"container/loom/pattern");
    private static final Identifier ERROR_SPRITE = Identifier.withDefaultNamespace((String)"container/loom/error");
    private static final Identifier BG_LOCATION = Identifier.withDefaultNamespace((String)"textures/gui/container/loom.png");
    ...
    private int totalRowCount() {
        return Mth.positiveCeilDiv((int)((LoomMenu)this.menu).getSelectablePatterns().size(), (int)4);
    }
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        if (this.displayPatterns) {
            int x = xo + 60;
            int y = yo + 13;
            List selectablePatterns = ((LoomMenu)this.menu).getSelectablePatterns();
            block0: for (int row = 0; row < 4; ++row) {
                for (int column = 0; column < 4; ++column) {
                    ...
                    int posY = y + row * 14;
                    Holder pattern = (Holder)selectablePatterns.get(index);
                    ...
                    if (index == ((LoomMenu)this.menu).getSelectedBannerPatternIndex()) {
                        ...
                        DyeColor patternColor = (DyeColor)this.dyeStack.getOrDefault(DataComponents.DYE, (Object)DyeColor.WHITE);
                        graphics.setTooltipForNextFrame((Component)Component.translatable((String)(((BannerPattern)pattern.value()).translationKey() + "." + patternColor.getName())), mouseX, mouseY);
                        graphics.requestCursor(CursorTypes.POINTING_HAND);
                    } else {
                        buttonSprite = PATTERN_SPRITE;
                    }
                    graphics.blitSprite(RenderPipelines.GUI_TEXTURED, buttonSprite, posX, posY, 14, 14);
                    TextureAtlasSprite bannerPatternSprite = graphics.getSprite(Sheets.getBannerSprite((Holder<BannerPattern>)pattern));
        ...
        if (this.displayPatterns) {
            ...
            for (int row = 0; row < 4; ++row) {
                for (int column = 0; column < 4; ++column) {
                    ...
                    int actualRow = row + this.startRow;
                    int index = actualRow * 4 + column;
                    if (!(xx >= 0.0) || !(yy >= 0.0) || !(xx < 14.0) || !(yy < 14.0) || !((LoomMenu)this.menu).clickMenuButton((Player)this.minecraft.player, index)) continue;
    ...
    private void containerChanged() {
        ...
        this.resultBannerPatterns = resultStack.isEmpty() ? null : (BannerPatternLayers)resultStack.getOrDefault(DataComponents.BANNER_PATTERNS, (Object)BannerPatternLayers.EMPTY);
        ...
        if (!(ItemStack.matches((ItemStack)bannerStack, (ItemStack)this.bannerStack) && ItemStack.matches((ItemStack)dyeStack, (ItemStack)this.dyeStack) && ItemStack.matches((ItemStack)patternStack, (ItemStack)this.patternStack))) {
