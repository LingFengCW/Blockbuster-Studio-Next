...
extends Screen {
    ...
    public AbstractSignEditScreen(SignBlockEntity sign, boolean isFrontText, boolean shouldFilter) {
        this(sign, isFrontText, shouldFilter, (Component)Component.translatable((String)"sign.edit"));
    }
    ...
    public AbstractSignEditScreen(SignBlockEntity sign, boolean isFrontText, boolean shouldFilter, Component title) {
        ...
        this.woodType = SignBlock.getWoodType((Block)sign.getBlockState().getBlock());
    ...
    public void removed() {
        ...
        if (connection != null) {
            connection.send((Packet<?>)new ServerboundSignUpdatePacket(this.sign.getBlockPos(), this.isFrontText, this.messages[0], this.messages[1], this.messages[2], this.messages[3]));
        }
    ...
    private void extractSignText(GuiGraphicsExtractor graphics, Vector2f cursorPosOutput) {
        ...
        for (i = 0; i < this.messages.length; ++i) {
            ...
            if (cursorPos < line.length()) {
                if (showCursor) {
                    TextCursorUtils.extractInsertCursor(graphics, cursorX, cursorY, ARGB.opaque((int)color), this.sign.getTextLineHeight());
                }
                cursorPosOutput.set((float)cursorX, (float)cursorY);
            }
    ...
    private void setMessage(String message) {
        ...
        this.text = this.text.setMessage(this.line, (Component)Component.literal((String)message));
