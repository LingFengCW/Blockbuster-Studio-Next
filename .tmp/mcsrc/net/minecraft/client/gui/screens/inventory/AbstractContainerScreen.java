...
implements MenuAccess<T> {
    public static final Identifier INVENTORY_LOCATION = Identifier.withDefaultNamespace((String)"textures/gui/container/inventory.png");
    ...
    protected void extractSlot(GuiGraphicsExtractor graphics, Slot slot, int mouseX, int mouseY) {
        ...
        if (this.isQuickCrafting && this.quickCraftSlots.contains(slot) && !carried.isEmpty()) {
            ...
            if (AbstractContainerMenu.canItemQuickReplace((Slot)slot, (ItemStack)carried, (boolean)true) && this.menu.canDragTo(slot)) {
                ...
                int newCount = AbstractContainerMenu.getQuickCraftPlaceCount((int)this.quickCraftSlots.size(), (int)this.quickCraftingType, (ItemStack)carried) + carry;
                if (newCount > maxSize) {
                    newCount = maxSize;
    ...
    private void recalculateQuickCraftRemaining() {
        ...
        for (Slot slot : this.quickCraftSlots) {
            ...
            int maxSize = Math.min(carried.getMaxStackSize(), slot.getMaxStackSize(carried));
            int newCount = Math.min(AbstractContainerMenu.getQuickCraftPlaceCount((int)this.quickCraftSlots.size(), (int)this.quickCraftingType, (ItemStack)carried) + carry, maxSize);
            ...
            if (event.hasShiftDown()) {
                if (!this.lastQuickMoved.isEmpty()) {
                    for (Slot target : ((AbstractContainerMenu)this.menu).slots) {
                        if (target == null || !target.mayPickup((Player)this.minecraft.player) || !target.hasItem() || target.container != slot.container || !AbstractContainerMenu.canItemQuickReplace((Slot)target, (ItemStack)this.lastQuickMoved, (boolean)true)) continue;
    ...
    public void removed() {
        ...
        this.menu.removed((Player)this.minecraft.player);
    }
    ...
    private boolean shouldAddSlotToQuickCraft(Slot slot, ItemStack carried) {
        return this.isQuickCrafting && !carried.isEmpty() && (carried.getCount() > this.quickCraftSlots.size() || this.quickCraftingType == 2) && AbstractContainerMenu.canItemQuickReplace((Slot)slot, (ItemStack)carried, (boolean)true) && slot.mayPlace(carried) && this.menu.canDragTo(slot);
    }

    private void quickCraftToSlots() {
        this.slotClicked(null, -999, AbstractContainerMenu.getQuickcraftMask((int)0, (int)this.quickCraftingType), ContainerInput.QUICK_CRAFT);
        for (Slot quickSlot : this.quickCraftSlots) {
            this.slotClicked(quickSlot, quickSlot.index, AbstractContainerMenu.getQuickcraftMask((int)1, (int)this.quickCraftingType), ContainerInput.QUICK_CRAFT);
        }
        this.slotClicked(null, -999, AbstractContainerMenu.getQuickcraftMask((int)2, (int)this.quickCraftingType), ContainerInput.QUICK_CRAFT);
    }
