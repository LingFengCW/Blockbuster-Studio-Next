 ...
 *  net.minecraft.world.level.gamerules.GameRuleMap$Builder
...
extends Screen {
    ...
    private static final Component GAME_MODEL_LABEL = Component.translatable((String)"selectWorld.gameMode");
    private static final Component NAME_LABEL = Component.translatable((String)"selectWorld.enterName");
    private static final Component EXPERIMENTS_LABEL = Component.translatable((String)"selectWorld.experiments");
    private static final Component ALLOW_COMMANDS_INFO = Component.translatable((String)"selectWorld.allowCommands.info");
    private static final Component PREPARING_WORLD_DATA = Component.translatable((String)"createWorld.preparing");
    private static final int HORIZONTAL_BUTTON_SPACING = 10;
    private static final int VERTICAL_BUTTON_SPACING = 8;
    public static final Identifier TAB_HEADER_BACKGROUND = Identifier.withDefaultNamespace((String)"textures/gui/tab_header_background.png");
    ...
    public static void openFresh(Minecraft minecraft, Runnable onClose) {
        CreateWorldScreen.openFresh(minecraft, onClose, (createWorldScreen, finalLayers, worldDataAndGenSettings, gameRules, tempDataPackDir) -> createWorldScreen.createNewWorld((LayeredRegistryAccess<RegistryLayer>)finalLayers, worldDataAndGenSettings, gameRules));
    }

    public static void openFresh(Minecraft minecraft, Runnable onClose, CreateWorldCallback createWorld) {
        WorldCreationContextMapper worldCreationContext = (managers, registries, cookie) -> new WorldCreationContext(cookie.worldGenSettings(), (LayeredRegistryAccess<RegistryLayer>)registries, managers, cookie.dataConfiguration());
        Function<WorldLoader.DataLoadContext, WorldGenSettings> worldGenSettings = context -> new WorldGenSettings(WorldOptions.defaultWithRandomSeed(), WorldPresets.createNormalWorldDimensions((HolderLookup.Provider)context.datapackWorldgen()));
        CreateWorldScreen.openCreateWorldScreen(minecraft, onClose, worldGenSettings, worldCreationContext, (ResourceKey<WorldPreset>)WorldPresets.NORMAL, createWorld);
    }

    public static void testWorld(Minecraft minecraft, Runnable onClose) {
        WorldCreationContextMapper worldCreationContext = (managers, registries, cookie) -> new WorldCreationContext(cookie.worldGenSettings().options(), cookie.worldGenSettings().dimensions(), (LayeredRegistryAccess<RegistryLayer>)registries, managers, cookie.dataConfiguration(), new InitialWorldCreationOptions(WorldCreationUiState.SelectedGameMode.CREATIVE, new GameRuleMap.Builder().set(GameRules.ADVANCE_TIME, (Object)false).set(GameRules.ADVANCE_WEATHER, (Object)false).set(GameRules.SPAWN_MOBS, (Object)false).build(), null));
        Function<WorldLoader.DataLoadContext, WorldGenSettings> worldGenSettings = context -> new WorldGenSettings(WorldOptions.testWorldWithRandomSeed(), WorldPresets.createTestWorldDimensions((HolderLookup.Provider)context.datapackWorldgen()));
        CreateWorldScreen.openCreateWorldScreen(minecraft, onClose, worldGenSettings, worldCreationContext, (ResourceKey<WorldPreset>)WorldPresets.FLAT_ALL_DIMENSIONS, (createWorldScreen, finalLayers, worldDataAndGenSettings, gameRules, tempDataPackDir) -> createWorldScreen.createNewWorld((LayeredRegistryAccess<RegistryLayer>)finalLayers, worldDataAndGenSettings, gameRules));
    }
    ...
    private static void openCreateWorldScreen(Minecraft minecraft, Runnable onClose, Function<WorldLoader.DataLoadContext, WorldGenSettings> worldGenSettings, WorldCreationContextMapper worldCreationContext, ResourceKey<WorldPreset> worldPreset, CreateWorldCallback createWorld) {
        ...
        CompletableFuture loadResult = WorldLoader.load((WorldLoader.InitConfig)loadConfig, context -> new WorldLoader.DataLoadOutput((Object)new DataPackReloadCookie((WorldGenSettings)worldGenSettings.apply(context), context.dataConfiguration()), context.datapackDimensions()), (resources, managers, registries, cookie) -> {
            resources.close();
            return worldCreationContext.apply(managers, (LayeredRegistryAccess<RegistryLayer>)registries, (DataPackReloadCookie)cookie);
        }, (Executor)Util.backgroundExecutor(), (Executor)((Object)minecraft));
    ...
    public static CreateWorldScreen createFromExisting(Minecraft minecraft, Runnable onClose, LevelSettings levelSettings, WorldCreationContext worldCreationContext, @Nullable Path newDataPackDir) {
        CreateWorldScreen result = new CreateWorldScreen(minecraft, onClose, worldCreationContext, WorldPresets.fromSettings((WorldDimensions)worldCreationContext.selectedDimensions()), OptionalLong.of(worldCreationContext.options().seed()), (createWorldScreen, finalLayers, worldDataAndGenSettings, gameRules, tempDataPackDir) -> createWorldScreen.createNewWorld((LayeredRegistryAccess<RegistryLayer>)finalLayers, worldDataAndGenSettings, gameRules));
    ...
    private CreateWorldScreen(Minecraft minecraft, Runnable onClose, WorldCreationContext settings, Optional<ResourceKey<WorldPreset>> preset, OptionalLong seed, CreateWorldCallback createWorldCallback) {
        super((Component)Component.translatable((String)"selectWorld.create"));
    ...
    protected void init() {
        ...
        footer.addChild(Button.builder((Component)Component.translatable((String)"selectWorld.create"), button -> this.onCreate()).build());
    ...
    private void onCreate() {
        ...
        LayeredRegistryAccess finalLayers = context.worldgenRegistries().replaceFrom((Object)RegistryLayer.DIMENSIONS, new RegistryAccess.Frozen[]{finalDimensions.dimensionsRegistryAccess()});
        FeatureFlagSet enabledFeatures = context.dataConfiguration().enabledFeatures();
        Lifecycle lifecycleFromFeatures = FeatureFlags.isExperimental((FeatureFlagSet)enabledFeatures) ? Lifecycle.experimental() : Lifecycle.stable();
        ...
        LevelDataAndDimensions.WorldDataAndGenSettings worldDataAndGenSettings = new LevelDataAndDimensions.WorldDataAndGenSettings((WorldData)worldData, worldGenSettings);
        WorldOpenFlows.confirmWorldCreation(this.minecraft, this, lifecycle, () -> this.createWorldAndCleanup((LayeredRegistryAccess<RegistryLayer>)finalLayers, worldDataAndGenSettings, Optional.of(gameRules)), skipWarning);
    }
    ...
    private void openDataPackSelectionScreen(WorldDataConfiguration dataConfiguration) {
        Pair<Path, PackRepository> settings = this.getDataPackSelectionSettings(dataConfiguration);
        if (settings != null) {
            this.minecraft.gui.setScreen(new PackSelectionScreen((PackRepository)settings.getSecond(), packRepository -> this.tryApplyNewDataPacks((PackRepository)packRepository, true, this::openDataPackSelectionScreen), (Path)settings.getFirst(), (Component)Component.translatable((String)"dataPack.title")));
        }
    }
    ...
    private void tryApplyNewDataPacks(PackRepository packRepository, boolean isDataPackScreen, Consumer<WorldDataConfiguration> onAbort) {
        List newDisabled;
        ImmutableList newEnabled = ImmutableList.copyOf((Collection)packRepository.getSelectedIds());
        WorldDataConfiguration newConfig = new WorldDataConfiguration(new DataPackConfig((List)newEnabled, newDisabled = (List)packRepository.getAvailableIds().stream().filter(arg_0 -> CreateWorldScreen.lambda$tryApplyNewDataPacks$0((List)newEnabled, arg_0)).collect(ImmutableList.toImmutableList())), this.uiState.getSettings().dataConfiguration().enabledFeatures());
        ...
        if (FeatureFlags.isExperimental((FeatureFlagSet)requestedFeatureFlags) && isDataPackScreen) {
    ...
    private void applyNewPackConfig(PackRepository packRepository, WorldDataConfiguration newConfig, Consumer<WorldDataConfiguration> onAbort) {
        this.minecraft.setScreenAndShow(new GenericMessageScreen((Component)Component.translatable((String)"dataPack.validation.working")));
        WorldLoader.InitConfig config = CreateWorldScreen.createDefaultLoadConfig(packRepository, newConfig);
        ((CompletableFuture)((CompletableFuture)WorldLoader.load((WorldLoader.InitConfig)config, context -> {
            ...
            RegistryOps writeOps = existingContext.worldgenLoadContext().createSerializationContext((DynamicOps)JsonOps.INSTANCE);
            DataResult encoded = WorldGenSettings.CODEC.encodeStart((DynamicOps)writeOps, (Object)new WorldGenSettings(existingContext.options(), existingContext.selectedDimensions())).setLifecycle(Lifecycle.stable());
            RegistryOps readOps = context.datapackWorldgen().createSerializationContext((DynamicOps)JsonOps.INSTANCE);
            WorldGenSettings settings = (WorldGenSettings)encoded.flatMap(arg_0 -> CreateWorldScreen.lambda$applyNewPackConfig$1((DynamicOps)readOps, arg_0)).getOrThrow(error -> new IllegalStateException("Error parsing worldgen settings after loading data packs: " + error));
            return new WorldLoader.DataLoadOutput((Object)new DataPackReloadCookie(settings, context.dataConfiguration()), context.datapackDimensions());
        }, (resources, managers, registries, cookie) -> {
            resources.close();
            return new WorldCreationContext(cookie.worldGenSettings(), (LayeredRegistryAccess<RegistryLayer>)registries, managers, cookie.dataConfiguration());
        }, (Executor)Util.backgroundExecutor(), (Executor)((Object)this.minecraft)).thenApply(settings -> {
            settings.validate();
            return settings;
        })).thenAcceptAsync(this.uiState::setSettings, (Executor)((Object)this.minecraft))).handleAsync((nothing, throwable) -> {
            if (throwable != null) {
                ...
                }, (Component)Component.translatable((String)"dataPack.validation.failed"), CommonComponents.EMPTY, (Component)Component.translatable((String)"dataPack.validation.back"), (Component)Component.translatable((String)"dataPack.validation.reset")));
            } else {
                this.minecraft.gui.setScreen(this);
            }
            return null;
    ...
    private static WorldLoader.InitConfig createDefaultLoadConfig(PackRepository packRepository, WorldDataConfiguration config) {
        ...
        return new WorldLoader.InitConfig(packConfig, Commands.CommandSelection.INTEGRATED, (PermissionSet)LevelBasedPermissionSet.GAMEMASTER);
    }
    ...
    private static void copyBetweenDirs(Path sourceDir, Path targetDir, Path sourcePath) {
        try {
            Util.copyBetweenDirs((Path)sourceDir, (Path)targetDir, (Path)sourcePath);
        }
        catch (IOException e) {
    ...
    private static Optional<LevelStorageSource.LevelStorageAccess> createNewWorldDirectory(Minecraft minecraft, String worldFolder, @Nullable Path tempDataPackDir) {
        ...
        block12: {
            ...
            try {
                ...
                FileUtil.createDirectoriesSafe((Path)targetDir);
    ...
    private @Nullable Pair<Path, PackRepository> getDataPackSelectionSettings(WorldDataConfiguration dataConfiguration) {
        ...
        if (dataPackDir != null) {
            if (this.tempDataPackRepository == null) {
                this.tempDataPackRepository = ServerPacksSource.createPackRepository((Path)dataPackDir, (DirectoryValidator)this.packValidator);
                this.tempDataPackRepository.reload();
            }
            this.tempDataPackRepository.setSelected((Collection)dataConfiguration.dataPacks().getEnabled());
            return Pair.of((Object)dataPackDir, (Object)this.tempDataPackRepository);
        }
        return null;
    }

    private static /* synthetic */ DataResult lambda$applyNewPackConfig$1(DynamicOps readOps, JsonElement r) {
        return WorldGenSettings.CODEC.parse(readOps, (Object)r);
    }
    ...
    extends GridLayoutTab {
        private static final Component TITLE = Component.translatable((String)"createWorld.tab.game.title");
        ...
        private GameTab(CreateWorldScreen createWorldScreen) {
            ...
            this.nameEdit = new EditBox(createWorldScreen.font, 208, 20, (Component)Component.translatable((String)"selectWorld.enterName"));
            this.nameEdit.setValue(createWorldScreen.uiState.getName());
            this.nameEdit.setResponder(createWorldScreen.uiState::setName);
            createWorldScreen.uiState.addListener(uiState -> this.nameEdit.setTooltip(Tooltip.create((Component)Component.translatable((String)"selectWorld.targetFolder", (Object[])new Object[]{Component.literal((String)uiState.getTargetFolder()).withStyle(ChatFormatting.ITALIC)}))));
            ...
            CycleButton<Difficulty> difficultyButton = helper.addChild(CycleButton.builder(Difficulty::getDisplayName, createWorldScreen.uiState.getDifficulty()).withValues((Difficulty[])Difficulty.values()).create(0, 0, 210, 20, (Component)Component.translatable((String)"options.difficulty"), (button, value) -> this$0.uiState.setDifficulty((Difficulty)value)), buttonLayoutSettings);
    ...
    extends GridLayoutTab {
        private static final Component TITLE = Component.translatable((String)"createWorld.tab.world.title");
        private static final Component AMPLIFIED_HELP_TEXT = Component.translatable((String)"generator.minecraft.amplified.info");
        private static final Component GENERATE_STRUCTURES = Component.translatable((String)"selectWorld.mapFeatures");
        private static final Component GENERATE_STRUCTURES_INFO = Component.translatable((String)"selectWorld.mapFeatures.info");
        private static final Component BONUS_CHEST = Component.translatable((String)"selectWorld.bonusItems");
        private static final Component SEED_LABEL = Component.translatable((String)"selectWorld.enterSeed");
        private static final Component SEED_EMPTY_HINT = Component.translatable((String)"selectWorld.seedInfo");
        ...
        private WorldTab(CreateWorldScreen createWorldScreen) {
            ...
            this.customizeTypeButton = helper.addChild(Button.builder((Component)Component.translatable((String)"selectWorld.customizeType"), b -> this.openPresetEditor()).build());
            createWorldScreen.uiState.addListener(data -> {
                this.customizeTypeButton.active = !data.isDebug() && data.getPresetEditor() != null;
            });
            this.seedEdit = new EditBox(this, createWorldScreen.font, 308, 20, (Component)Component.translatable((String)"selectWorld.enterSeed")){
                {
    ...
    extends GridLayoutTab {
        private static final Component TITLE = Component.translatable((String)"createWorld.tab.more.title");
        private static final Component GAME_RULES_LABEL = Component.translatable((String)"selectWorld.gameRules");
        private static final Component DATA_PACKS_LABEL = Component.translatable((String)"selectWorld.dataPacks");
        final /* synthetic */ CreateWorldScreen this$0;
