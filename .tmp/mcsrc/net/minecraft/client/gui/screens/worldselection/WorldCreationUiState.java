...
public class WorldCreationUiState {
    private static final Component DEFAULT_WORLD_NAME = Component.translatable((String)"selectWorld.newWorld");
    ...
    public WorldCreationUiState(Path savesFolder, WorldCreationContext settings, Optional<ResourceKey<WorldPreset>> preset, OptionalLong seed) {
        ...
        this.worldType = new WorldTypeEntry((Holder<WorldPreset>)((Holder)WorldCreationUiState.findPreset(settings, preset).orElse(null)));
    ...
    private String findResultFolder(String name) {
        if (!Files.exists(this.savesFolder, new LinkOption[0])) {
            try {
                ...
                throw new LevelStorageException((Component)Component.literal((String)"Could not create save folder"), (Exception)e);
            }
        }
        String trimmedName = name.trim();
        try {
            return FileUtil.findAvailableName((Path)this.savesFolder, (String)(!trimmedName.isEmpty() ? trimmedName : DEFAULT_WORLD_NAME.getString()), (String)"");
        }
        catch (Exception exception) {
            try {
                return FileUtil.findAvailableName((Path)this.savesFolder, (String)"World", (String)"");
            }
            catch (IOException e) {
                throw new LevelStorageException((Component)Component.literal((String)"Could not create world folder"), (Exception)e);
            }
        }
    }
    ...
    public void setSeed(String seed) {
        this.seed = seed;
        this.settings = this.settings.withOptions(options -> options.withSeed(WorldOptions.parseSeed((String)this.getSeed())));
        this.onChanged();
    }
    ...
    protected boolean tryUpdateDataConfiguration(WorldDataConfiguration newConfig) {
        ...
        if (oldConfig.dataPacks().getEnabled().equals(newConfig.dataPacks().getEnabled()) && oldConfig.enabledFeatures().equals((Object)newConfig.enabledFeatures())) {
    ...
    private void updatePresetLists() {
        ...
        this.normalPresetList.addAll(WorldCreationUiState.getNonEmptyList((Registry<WorldPreset>)presetRegistry, (TagKey<WorldPreset>)WorldPresetTags.NORMAL).orElseGet(() -> presetRegistry.listElements().map(WorldTypeEntry::new).toList()));
        this.altPresetList.clear();
        this.altPresetList.addAll((Collection<WorldTypeEntry>)WorldCreationUiState.getNonEmptyList((Registry<WorldPreset>)presetRegistry, (TagKey<WorldPreset>)WorldPresetTags.EXTENDED).orElse(this.normalPresetList));
    ...
    public static enum SelectedGameMode {
        ...
        private SelectedGameMode(String name, GameType gameType) {
            ...
            this.displayName = Component.translatable((String)("selectWorld.gameMode." + name));
            this.info = Component.translatable((String)("selectWorld.gameMode." + name + ".info"));
        }
    ...
    public record WorldTypeEntry(@Nullable Holder<WorldPreset> preset) {
        private static final Component CUSTOM_WORLD_DESCRIPTION = Component.translatable((String)"generator.custom");

        public Component describePreset() {
            return Optional.ofNullable(this.preset).flatMap(Holder::unwrapKey).map(key -> Component.translatable((String)key.identifier().toLanguageKey("generator"))).orElse(CUSTOM_WORLD_DESCRIPTION);
        }
