...
extends Screen {
    ...
    private static final Component USAGE_TEXT = Component.translatable((String)"chat_screen.usage");
    private static final Component RESTRICTED_NARRATION_TEXT = Component.translatable((String)"chat_screen.restricted.narration");
    ...
    public ChatScreen(String initial, boolean isDraft, boolean closeOnSubmit) {
        super((Component)Component.translatable((String)"chat_screen.title"));
    ...
    protected void init() {
        ...
        this.input = new EditBox(this, this.minecraft.fontFilterFishy, 4, this.height - 12, this.width - 4, 12, (Component)Component.translatable((String)"chat.editBox")){
    ...
    public boolean mouseScrolled(double x, double y, double scrollX, double scrollY) {
        if (this.commandSuggestions.mouseScrolled(scrollY = Mth.clamp((double)scrollY, (double)-1.0, (double)1.0))) {
            return true;
        }
            ...
            block4: while (true) {
                switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{ClickEvent.Custom.class, ClickEvent.Custom.class}, (ClickEvent)clickEvent2, n)) {
                    case 0: {
                        ...
                        if (!customEvent.id().equals((Object)ChatComponent.QUEUE_EXPAND_ID)) {
                            n = 1;
                            continue block4;
                        }
                    ...
                    case 1: {
                        ClickEvent.Custom customEvent = (ClickEvent.Custom)clickEvent2;
                        if (!customEvent.id().equals((Object)ChatComponent.GO_TO_RESTRICTIONS_SCREEN)) {
                            n = 2;
                            continue block4;
                        }
    ...
    public void moveInHistory(int dir) {
        ...
        if ((newPos = Mth.clamp((int)newPos, (int)0, (int)max)) == this.historyPos) {
            return;
        }
        if (newPos == max) {
            this.historyPos = max;
    ...
    private @Nullable FormattedCharSequence formatChat(String text, int offset) {
        if (this.isDraft) {
            return FormattedCharSequence.forward((String)text, (Style)Style.EMPTY.withColor(ChatFormatting.GRAY).withItalic(Boolean.valueOf(true)));
        }
        return null;
    }
    ...
    protected void updateNarrationState(NarrationElementOutput output) {
        ...
        if (this.displayMode.showRestrictedPrompt) {
            output.add(NarratedElementType.USAGE, (Component)CommonComponents.joinForNarration((Component[])new Component[]{USAGE_TEXT, RESTRICTED_NARRATION_TEXT}));
        } else {
            output.add(NarratedElementType.USAGE, USAGE_TEXT);
        }
        String value = this.input.getValue();
        if (!value.isEmpty()) {
            output.nest().add(NarratedElementType.TITLE, (Component)Component.translatable((String)"chat_screen.message", (Object[])new Object[]{value}));
        }
    }
    ...
    public String normalizeChatMessage(String message) {
        return StringUtil.trimChatMessage((String)StringUtils.normalizeSpace((String)message.trim()));
    }
