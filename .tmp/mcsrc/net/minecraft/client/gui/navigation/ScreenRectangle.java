...
public record ScreenRectangle(ScreenPosition position, int width, int height) {
    ...
    public ScreenRectangle transformAxisAligned(Matrix3x2fc matrix) {
        ...
        return new ScreenRectangle(Mth.floor((float)topLeft.x), Mth.floor((float)topLeft.y), Mth.floor((float)(bottomRight.x - topLeft.x)), Mth.floor((float)(bottomRight.y - topLeft.y)));
    }
