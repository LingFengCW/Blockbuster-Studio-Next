...
public class KeyframeAnimation {
    ...
    private record Entry(ModelPart part, AnimationChannel.Target target, Keyframe[] keyframes) {
        public void apply(float secondsSinceStart, float targetScale, Vector3f scratchVector) {
            int prev = Math.max(0, Mth.binarySearch((int)0, (int)this.keyframes.length, i -> secondsSinceStart <= this.keyframes[i].timestamp()) - 1);
            ...
            float lerpAlpha = next != prev ? Mth.clamp((float)(keyframeTimeDelta / (nextFrame.timestamp() - previousFrame.timestamp())), (float)0.0f, (float)1.0f) : 0.0f;
