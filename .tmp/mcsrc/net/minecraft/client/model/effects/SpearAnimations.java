...
public class SpearAnimations {
    private static float progress(float time, float start, float end) {
        return Mth.clamp((float)Mth.inverseLerp((float)time, (float)start, (float)end), (float)0.0f, (float)1.0f);
    }
    ...
    public static <S extends ArmedEntityRenderState> void thirdPersonUseItem(S state, PoseStack poseStack, float timeHeld, HumanoidArm arm, ItemStack actualItem) {
        ...
        float attack = Ease.inQuad((float)SpearAnimations.progress(state.attackTime, 0.05f, 0.2f));
        float retract = Ease.inOutExpo((float)SpearAnimations.progress(state.attackTime, 0.4f, 1.0f));
        UseParams params = UseParams.fromKineticWeapon(kineticWeapon, timeHeld);
        int invert = arm == HumanoidArm.RIGHT ? 1 : -1;
        float raiseProgressModified = 1.0f - Ease.outBack((float)(1.0f - params.raiseProgress()));
    ...
    public static <T extends HumanoidRenderState> void thirdPersonAttackHand(HumanoidModel<T> model, T state) {
        ...
        float prepare = Ease.inOutSine((float)SpearAnimations.progress(attackTime, 0.0f, 0.05f));
        float attack = Ease.inQuad((float)SpearAnimations.progress(attackTime, 0.05f, 0.2f));
        float retract = Ease.inOutExpo((float)SpearAnimations.progress(attackTime, 0.4f, 1.0f));
    ...
    private static float hitFeedbackAmount(float ticksSinceFeedbackStart) {
        return 0.4f * (Ease.outQuart((float)SpearAnimations.progress(ticksSinceFeedbackStart, 1.0f, 3.0f)) - Ease.inOutSine((float)SpearAnimations.progress(ticksSinceFeedbackStart, 3.0f, 10.0f)));
    }
    ...
    public static void firstPersonUse(float ticksSinceKineticHitFeedback, PoseStack poseStack, float timeHeld, HumanoidArm arm, ItemStack itemStack) {
        ...
        poseStack.rotateAround((Quaternionfc)Axis.XP.rotationDegrees(-65.0f * Ease.inOutBack((float)params.raiseProgress()) - 35.0f * params.lowerProgress() + 100.0f * params.raiseBackProgress() + -0.5f * params.swayScaleFast()), 0.0f, 0.1f, 0.0f);
    ...
    public static void firstPersonAttack(float attack, PoseStack poseStack, int invert, HumanoidArm arm) {
        float startingAmount = Ease.inOutSine((float)SpearAnimations.progress(attack, 0.0f, 0.05f));
        float middleAmount = Ease.outBack((float)SpearAnimations.progress(attack, 0.05f, 0.2f));
        float endingAmount = Ease.inOutExpo((float)SpearAnimations.progress(attack, 0.4f, 1.0f));
    ...
    private record UseParams(float raiseProgress, float raiseProgressStart, float raiseProgressMiddle, float raiseProgressEnd, float swayProgress, float lowerProgress, float raiseBackProgress, float swayIntensity, float swayScaleSlow, float swayScaleFast) {
        public static UseParams fromKineticWeapon(KineticWeapon kineticWeapon, float time) {
            ...
            float lowerProgress = Ease.outCubic((float)Ease.inOutElastic((float)SpearAnimations.progress(time - 20.0f, startLoweringTick, finishLoweringTick)));
            float raiseBackProgress = SpearAnimations.progress(time, finishRaisingBackTick - 5, finishRaisingBackTick);
            float swayIntensity = 2.0f * Ease.outCirc((float)swayProgress) - 2.0f * Ease.inCirc((float)raiseBackProgress);
            float swayScaleSlow = Mth.sin((double)(time * 19.0f * ((float)Math.PI / 180))) * swayIntensity;
            float swayScaleFast = Mth.sin((double)(time * 30.0f * ((float)Math.PI / 180))) * swayIntensity;
