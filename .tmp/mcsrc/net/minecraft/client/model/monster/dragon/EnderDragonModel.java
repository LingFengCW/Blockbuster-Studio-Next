...
extends EntityModel<EnderDragonRenderState> {
    ...
    public void setupAnim(EnderDragonRenderState state) {
        ...
        float rot2 = Mth.wrapDegrees((float)(state.getHistoricalPos(5).yRot() - state.getHistoricalPos(10).yRot()));
        float rot = Mth.wrapDegrees((float)(state.getHistoricalPos(5).yRot() + rot2 / 2.0f));
        for (int i = 0; i < 5; ++i) {
            ...
            neck.yRot = Mth.wrapDegrees((float)(point.yRot() - start.yRot())) * ((float)Math.PI / 180) * 1.5f;
            neck.xRot = neckXRot + state.getHeadPartYOffset(i, start, point) * ((float)Math.PI / 180) * 1.5f * 5.0f;
            neck.zRot = -Mth.wrapDegrees((float)(point.yRot() - rot)) * ((float)Math.PI / 180) * 1.5f;
            neck.y = yy;
            neck.z = zz;
            neck.x = xx;
        ...
        this.head.yRot = Mth.wrapDegrees((float)(current.yRot() - start.yRot())) * ((float)Math.PI / 180);
        this.head.xRot = Mth.wrapDegrees((float)state.getHeadPartYOffset(6, start, current)) * ((float)Math.PI / 180) * 1.5f * 5.0f;
        this.head.zRot = -Mth.wrapDegrees((float)(current.yRot() - rot)) * ((float)Math.PI / 180);
        ...
        for (int i = 0; i < 12; ++i) {
            ...
            tail.yRot = (Mth.wrapDegrees((float)(point.yRot() - start.yRot())) * 1.5f + 180.0f) * ((float)Math.PI / 180);
            tail.xRot = (tailXRot += Mth.sin((double)((float)i * 0.45f + flapTime)) * 0.05f) + (float)(point.y() - start.y()) * ((float)Math.PI / 180) * 1.5f * 5.0f;
            tail.zRot = Mth.wrapDegrees((float)(point.yRot() - rot)) * ((float)Math.PI / 180) * 1.5f;
            tail.y = yy;
            tail.z = zz;
            tail.x = xx;
