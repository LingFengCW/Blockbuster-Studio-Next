...
extends EntityModel<RavagerRenderState> {
    ...
    public void setupAnim(RavagerRenderState state) {
        ...
        int attackTime = 10;
        if (attackTick > 0.0f) {
            float headAnim = Mth.triangleWave((float)attackTick, (float)10.0f);
            ...
            this.mouth.xRot = attackTick > 5.0f ? Mth.sin((double)((-4.0f + attackTick) / 4.0f)) * (float)Math.PI * 0.4f : 0.15707964f * Mth.sin((double)((float)Math.PI * attackTick / 10.0f));
        } else {
            float headPos = -1.0f;
