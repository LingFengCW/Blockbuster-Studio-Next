...
extends EntityModel<HoglinRenderState> {
    ...
    public void setupAnim(HoglinRenderState state) {
        ...
        float headbuttLerpFactor = 1.0f - (float)Mth.abs((int)(10 - 2 * state.attackAnimationRemainingTicks)) / 10.0f;
        ...
        this.rightHindLeg.xRot = this.leftFrontLeg.xRot = Mth.cos((double)(animationPos + (float)Math.PI)) * 1.2f * animationSpeed;
        this.leftHindLeg.xRot = this.rightFrontLeg.xRot;
    }

    protected void animateHeadbutt(float headbuttLerpFactor) {
        this.head.xRot = Mth.lerp((float)headbuttLerpFactor, (float)0.87266463f, (float)-0.34906584f);
    }
}
