...
extends EntityModel<LivingEntityRenderState> {
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        for (i = 0; i < 4; ++i) {
            x = Mth.cos((double)angle) * 9.0f;
            y = -2.0f + Mth.cos((double)((float)(i * 2) * 0.25f));
            z = Mth.sin((double)angle) * 9.0f;
            root.addOrReplaceChild(BlazeModel.getPartName(i), rod, PartPose.offset(x, y, z));
            angle += 1.5707964f;
        }
        angle = 0.7853982f;
        for (i = 4; i < 8; ++i) {
            x = Mth.cos((double)angle) * 7.0f;
            y = 2.0f + Mth.cos((double)((float)(i * 2) * 0.25f));
            z = Mth.sin((double)angle) * 7.0f;
            root.addOrReplaceChild(BlazeModel.getPartName(i), rod, PartPose.offset(x, y, z));
            angle += 1.5707964f;
        }
        angle = 0.47123894f;
        for (i = 8; i < 12; ++i) {
            x = Mth.cos((double)angle) * 5.0f;
            y = 11.0f + Mth.cos((double)((float)i * 1.5f * 0.5f));
            z = Mth.sin((double)angle) * 5.0f;
            root.addOrReplaceChild(BlazeModel.getPartName(i), rod, PartPose.offset(x, y, z));
            angle += 1.5707964f;
        }
