...
extends EntityModel<ArmadilloRenderState> {
    ...
    public void setupAnim(ArmadilloRenderState state) {
        ...
        if (state.isHidingInShell) {
            ...
            this.head.xRot = Mth.clamp((float)state.xRot, (float)-22.5f, (float)25.0f) * ((float)Math.PI / 180);
            this.head.yRot = Mth.clamp((float)state.yRot, (float)-32.5f, (float)32.5f) * ((float)Math.PI / 180);
        }
        if (!state.isHidingInShell) {
