...
implements ArmedModel<AllayRenderState> {
    ...
    public void setupAnim(AllayRenderState state) {
        ...
        if (state.isDancing) {
            ...
            float danceFrequency = Mth.cos((double)danceSpeed) * 16.0f * ((float)Math.PI / 180);
            float spinningRotation = state.spinningProgress;
            float headTiltZ = Mth.cos((double)danceSpeed) * 14.0f * ((float)Math.PI / 180);
            float headTiltY = Mth.cos((double)danceSpeed) * 30.0f * ((float)Math.PI / 180);
        ...
        float armFlyingRotX = holdingItemFactor * Mth.lerp((float)flyingFactor, (float)-1.0471976f, (float)-1.134464f);
        ...
        float armIdleBobAmount = 0.43633232f - Mth.cos((double)(idleBobSpeed + 4.712389f)) * (float)Math.PI * 0.075f * armIdleBobFactor;
