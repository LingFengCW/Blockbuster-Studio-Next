...
extends EntityModel<T> {
    ...
    public void setupAnim(T state) {
        ...
        float clampedYRot = Mth.clamp((float)((EquineRenderState)state).yRot, (float)-20.0f, (float)20.0f);
        ...
        float baseHeadAngle = (1.0f - Math.max(standing, eating)) * (0.5235988f + headRotXRad + feedingAnim * Mth.sin((double)((EquineRenderState)state).ageInTicks) * 0.05f);
        this.headParts.xRot = standing * (0.2617994f + headRotXRad) + eating * (2.1816616f + Mth.sin((double)((EquineRenderState)state).ageInTicks) * 0.05f) + baseHeadAngle;
        ...
        float bobValue = Mth.cos((double)(((EquineRenderState)state).ageInTicks * 0.6f + (float)Math.PI));
        ...
        this.tail.yRot = animateTail ? Mth.cos((double)(((EquineRenderState)state).ageInTicks * 0.7f)) : 0.0f;
    }
    ...
    protected void animateHeadPartsPlacement(float eating, float standing) {
        this.headParts.y += Mth.lerp((float)eating, (float)Mth.lerp((float)standing, (float)0.0f, (float)-8.0f), (float)7.0f);
        this.headParts.z = Mth.lerp((float)standing, (float)this.headParts.z, (float)-4.0f);
    }
}
