    ...
    public void setupAnim(S state) {
        ...
        if (!((FelineRenderState)state).isSitting) {
            ...
            if (((FelineRenderState)state).isSprinting) {
                ...
                this.tail2.xRot = !((FelineRenderState)state).isCrouching ? 1.7278761f + 0.7853982f * Mth.cos((double)animationPos) * animationSpeed : 1.7278761f + 0.47123894f * Mth.cos((double)animationPos) * animationSpeed;
            }
        ...
        if (((FelineRenderState)state).lieDownAmount > 0.0f) {
            this.body.x += 1.0f;
            this.head.xRot = Mth.rotLerp((float)((FelineRenderState)state).lieDownAmount, (float)this.head.xRot, (float)0.17453292f);
            this.head.zRot = Mth.rotLerp((float)((FelineRenderState)state).lieDownAmount, (float)this.head.zRot, (float)-1.3089969f);
            this.head.x += 1.5f;
            this.head.y += 0.75f;
            this.head.z -= 0.5f;
            ...
            this.tail1.xRot += Mth.rotLerp((float)((FelineRenderState)state).lieDownAmountTail, (float)this.tail1.xRot, (float)-0.5235988f);
            this.tail1.yRot += Mth.rotLerp((float)((FelineRenderState)state).lieDownAmountTail, (float)this.tail1.yRot, (float)0.0f);
            this.tail1.zRot += Mth.rotLerp((float)((FelineRenderState)state).lieDownAmountTail, (float)this.tail1.zRot, (float)-0.17453292f);
            this.tail1.x += 1.0f;
            this.tail1.y += 0.5f;
            this.tail1.z -= 0.25f;
        }
        if (((FelineRenderState)state).relaxStateOneAmount > 0.0f) {
            this.head.xRot = Mth.rotLerp((float)((FelineRenderState)state).relaxStateOneAmount, (float)this.head.xRot, (float)-0.58177644f);
        }
    }
}
