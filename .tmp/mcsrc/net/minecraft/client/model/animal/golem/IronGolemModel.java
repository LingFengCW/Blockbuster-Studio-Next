...
extends EntityModel<IronGolemRenderState> {
    ...
    public void setupAnim(IronGolemRenderState state) {
        ...
        if (attackTick > 0.0f) {
            this.rightArm.xRot = -2.0f + 1.5f * Mth.triangleWave((float)attackTick, (float)10.0f);
            this.leftArm.xRot = -2.0f + 1.5f * Mth.triangleWave((float)attackTick, (float)10.0f);
        } else {
            int offerFlowerTick = state.offerFlowerTick;
            if (offerFlowerTick > 0) {
                this.rightArm.xRot = -0.8f + 0.025f * Mth.triangleWave((float)offerFlowerTick, (float)70.0f);
                this.leftArm.xRot = 0.0f;
            } else {
                this.rightArm.xRot = (-0.2f + 1.5f * Mth.triangleWave((float)animationPos, (float)13.0f)) * animationSpeed;
                this.leftArm.xRot = (-0.2f - 1.5f * Mth.triangleWave((float)animationPos, (float)13.0f)) * animationSpeed;
            }
        }
        this.head.yRot = state.yRot * ((float)Math.PI / 180);
        this.head.xRot = state.xRot * ((float)Math.PI / 180);
        this.rightLeg.xRot = -1.5f * Mth.triangleWave((float)animationPos, (float)13.0f) * animationSpeed;
        this.leftLeg.xRot = 1.5f * Mth.triangleWave((float)animationPos, (float)13.0f) * animationSpeed;
