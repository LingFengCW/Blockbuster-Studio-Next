...
extends QuadrupedModel<PandaRenderState> {
    ...
    public void setupAnim(PandaRenderState state) {
        ...
        if (state.lieOnBackAmount > 0.0f) {
            ...
            this.head.xRot = Mth.rotLerpRad((float)state.lieOnBackAmount, (float)this.head.xRot, (float)1.5707964f);
        }
        if (state.rollAmount > 0.0f) {
            this.head.xRot = Mth.rotLerpRad((float)state.rollAmount, (float)this.head.xRot, (float)2.0561945f);
    ...
    protected void animateSitting(PandaRenderState state) {
        this.body.xRot = Mth.rotLerpRad((float)state.sitAmount, (float)this.body.xRot, (float)1.7407963f);
        this.head.xRot = Mth.rotLerpRad((float)state.sitAmount, (float)this.head.xRot, (float)1.5707964f);
