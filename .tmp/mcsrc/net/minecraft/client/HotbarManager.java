...
public class HotbarManager {
    ...
    private void load() {
        try {
            CompoundTag tag = NbtIo.read((Path)this.optionsFile);
            if (tag == null) {
                return;
            }
            int version = NbtUtils.getDataVersion((CompoundTag)tag, (int)1343);
    ...
    public void save() {
        try {
            CompoundTag tag = NbtUtils.addCurrentDataVersion((CompoundTag)new CompoundTag());
            for (int i = 0; i < 9; ++i) {
            ...
            NbtIo.write((CompoundTag)tag, (Path)this.optionsFile);
        }
        catch (Exception e) {
