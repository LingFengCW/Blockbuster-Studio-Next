...
public class SessionSearchTrees {
    ...
    private static Stream<String> getTooltipLines(Stream<ItemStack> items, Item.TooltipContext context, TooltipFlag flag) {
        return items.flatMap(item -> item.getTooltipLines(context, null, flag).stream()).map(l -> ChatFormatting.stripFormatting((String)l.getString()).trim()).filter(s -> !s.isEmpty());
    }
    ...
    public void updateRecipes(ClientRecipeBook recipeBook, Level level) {
        this.register(RECIPE_COLLECTIONS, () -> {
            ...
            Registry itemRegistries = registryAccess.lookupOrThrow(Registries.ITEM);
            Item.TooltipContext tooltipContext = Item.TooltipContext.of((HolderLookup.Provider)registryAccess);
            ContextMap recipeContext = SlotDisplayContext.fromLevel((Level)level);
            ...
            this.recipeSearch = CompletableFuture.supplyAsync(() -> SessionSearchTrees.lambda$updateRecipes$1(recipeContext, tooltipContext, (TooltipFlag)tooltipFlag, itemRegistries, recipes), (Executor)Util.backgroundExecutor());
    ...
    public void updateCreativeTags(List<ItemStack> items) {
        this.register(CREATIVE_TAGS, () -> {
            ...
            this.creativeByTagSearch = CompletableFuture.supplyAsync(() -> new IdSearchTree<ItemStack>(itemStack -> itemStack.tags().map(TagKey::location), items), (Executor)Util.backgroundExecutor());
            previous.cancel(true);
        });
    }
    ...
    public void updateCreativeTooltips(HolderLookup.Provider registries, List<ItemStack> itemStacks) {
        this.register(CREATIVE_NAMES, () -> {
            Item.TooltipContext tooltipContext = Item.TooltipContext.of((HolderLookup.Provider)registries);
            ...
            this.creativeByNameSearch = CompletableFuture.supplyAsync(() -> SessionSearchTrees.lambda$updateCreativeTooltips$1(tooltipContext, (TooltipFlag)tooltipFlag, itemStacks), (Executor)Util.backgroundExecutor());
            previous.cancel(true);
        });
    }
    ...
    private static /* synthetic */ SearchTree lambda$updateCreativeTooltips$1(Item.TooltipContext tooltipContext, TooltipFlag tooltipFlag, List itemStacks) {
        return new FullTextSearchTree<ItemStack>(itemStack -> SessionSearchTrees.getTooltipLines(Stream.of(itemStack), tooltipContext, tooltipFlag), itemStack -> itemStack.typeHolder().unwrapKey().map(ResourceKey::identifier).stream(), itemStacks);
    }
    ...
    private static /* synthetic */ SearchTree lambda$updateRecipes$1(ContextMap recipeContext, Item.TooltipContext tooltipContext, TooltipFlag tooltipFlag, Registry itemRegistries, List recipes) {
        return new FullTextSearchTree<RecipeCollection>(collection -> SessionSearchTrees.getTooltipLines(collection.getRecipes().stream().flatMap(e -> e.resultItems(recipeContext).stream()), tooltipContext, tooltipFlag), collection -> collection.getRecipes().stream().flatMap(e -> e.resultItems(recipeContext).stream()).map(stack -> itemRegistries.getKey((Object)stack.getItem())), recipes);
    }
