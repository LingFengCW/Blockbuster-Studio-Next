...
public class ClientDebugSubscriber {
    ...
    private void onSubscriptionsChanged(Set<DebugSubscription<?>> newSubscriptions) {
        ...
        this.connection.send((Packet<?>)new ServerboundDebugSubscriptionRequestPacket(newSubscriptions));
    }
    ...
    public <T> void pushEvent(long gameTime, DebugSubscription.Event<T> event) {
        ValueMaps values = this.getValueMaps(event.subscription());
        if (values != null) {
            values.events.add(new ValueWrapper<Object>(event.value(), gameTime + (long)event.subscription().expireAfterTicks()));
        }
    }
    ...
    private static class ValueMaps<V> {
        ...
        public void dropChunkAndBlocks(ChunkPos chunkPos) {
            ...
            this.blockValues.removeKeys(arg_0 -> ((ChunkPos)chunkPos).contains(arg_0));
        }
    }
