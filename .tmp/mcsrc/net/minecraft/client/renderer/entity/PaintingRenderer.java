...
extends EntityRenderer<Painting, PaintingRenderState> {
    private static final Identifier BACK_SPRITE_LOCATION = Identifier.withDefaultNamespace((String)"back");
    ...
    public void extractRenderState(Painting entity, PaintingRenderState state, float partialTicks) {
        ...
        for (int segmentY = 0; segmentY < height; ++segmentY) {
            for (int segmentX = 0; segmentX < width; ++segmentX) {
                ...
                int y = Mth.floor((double)(entity.getY() + (double)segmentOffsetY));
                int z = entity.getBlockZ();
                switch (direction) {
                    case NORTH: {
                        x = Mth.floor((double)(entity.getX() + (double)segmentOffsetX));
                        break;
                    }
                    case WEST: {
                        z = Mth.floor((double)(entity.getZ() - (double)segmentOffsetX));
                        break;
                    }
                    case SOUTH: {
                        x = Mth.floor((double)(entity.getX() - (double)segmentOffsetX));
                ...
                state.lightCoordsPerBlock[segmentX + segmentY * width] = LightCoordsUtil.getLightCoords((BlockAndLightGetter)level, (BlockPos)new BlockPos(x, y, z));
            }
        }
    }
