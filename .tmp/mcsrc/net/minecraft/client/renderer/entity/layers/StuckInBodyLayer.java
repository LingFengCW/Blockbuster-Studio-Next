...
extends RenderLayer<AvatarRenderState, M> {
    ...
    private void submitStuckItem(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, float directionX, float directionY, float directionZ, int outlineColor) {
        float directionXZ = Mth.sqrt((float)(directionX * directionX + directionZ * directionZ));
    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, AvatarRenderState state, float yRot, float xRot) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            poseStack.translate(Mth.lerp((float)midX, (float)cube.minX, (float)cube.maxX) / 16.0f, Mth.lerp((float)midY, (float)cube.minY, (float)cube.maxY) / 16.0f, Mth.lerp((float)midZ, (float)cube.minZ, (float)cube.maxZ) / 16.0f);
