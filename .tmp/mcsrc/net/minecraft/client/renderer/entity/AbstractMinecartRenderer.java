...
extends EntityRenderer<T, S> {
    private static final Identifier MINECART_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/minecart/minecart.png");
    ...
    public void submit(S state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera) {
        ...
        if (hurt > 0.0f) {
            poseStack.mulPose((Quaternionfc)Axis.XP.rotationDegrees(Mth.sin((double)hurt) * hurt * ((MinecartRenderState)state).damageTime / 10.0f * (float)((MinecartRenderState)state).hurtDir));
        }
    ...
    public void extractRenderState(T entity, S state, float partialTicks) {
        ...
        ((MinecartRenderState)state).hurtTime = (float)entity.getHurtTime() - partialTicks;
        ((MinecartRenderState)state).hurtDir = entity.getHurtDir();
        ((MinecartRenderState)state).damageTime = Math.max(entity.getDamage() - partialTicks, 0.0f);
        ((MinecartRenderState)state).displayOffset = entity.getDisplayOffset();
        this.blockModelResolver.update(((MinecartRenderState)state).displayBlockModel, entity.getDisplayBlockState(), BLOCK_DISPLAY_CONTEXT);
    }
    ...
    protected AABB getBoundingBoxForCulling(T entity) {
        ...
        if (!entity.getDisplayBlockState().isAir()) {
            return aabb.expandTowards(0.0, (double)((float)entity.getDisplayOffset() * 0.75f / 16.0f), 0.0);
        }
        return aabb;
    }
