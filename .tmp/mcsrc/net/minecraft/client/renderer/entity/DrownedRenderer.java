...
extends AbstractZombieRenderer<Drowned, ZombieRenderState, DrownedModel> {
    private static final Identifier DROWNED_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/zombie/drowned.png");
    private static final Identifier BABY_DROWNED_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/zombie/drowned_baby.png");
    ...
    protected void setupRotations(ZombieRenderState state, PoseStack poseStack, float bodyRot, float entityScale) {
        ...
        if (swimAmount > 0.0f) {
            ...
            float rotationX = Mth.lerp((float)swimAmount, (float)0.0f, (float)targetRotationX);
    ...
    protected HumanoidModel.ArmPose getArmPose(Drowned mob, HumanoidArm arm) {
        ...
        if (mob.getMainArm() == arm && mob.isAggressive() && item.is((Object)Items.TRIDENT)) {
