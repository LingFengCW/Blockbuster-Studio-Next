...
extends EntityRenderer<T, ItemFrameRenderState> {
    ...
    protected int getBlockLightLevel(T entity, BlockPos blockPos) {
        if (entity.is((Object)EntityTypes.GLOW_ITEM_FRAME)) {
    ...
    public void extractRenderState(T entity, ItemFrameRenderState state, float partialTicks) {
        ...
        this.itemModelResolver.updateForNonLiving(state.item, itemStack, ItemDisplayContext.FIXED, (Entity)entity);
        state.rotation = entity.getRotation();
        state.isGlowFrame = entity.is((Object)EntityTypes.GLOW_ITEM_FRAME);
        state.mapId = null;
        if (!itemStack.isEmpty() && (framedMapId = entity.getFramedMapId(itemStack)) != null && (mapData = entity.level().getMapData(framedMapId)) != null) {
