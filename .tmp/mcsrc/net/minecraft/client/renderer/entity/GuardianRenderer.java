...
extends MobRenderer<Guardian, GuardianRenderState, GuardianModel> {
    private static final Identifier GUARDIAN_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/guardian/guardian.png");
    private static final Identifier GUARDIAN_BEAM_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/guardian/guardian_beam.png");
    ...
    public boolean shouldRender(Guardian entity, Frustum culler, double camX, double camY, double camZ) {
        ...
        if (entity.hasActiveAttackTarget() && (lookAtEntity = entity.getActiveAttackTarget()) != null) {
            ...
            Vec3 startPos = this.getPosition((LivingEntity)entity, entity.getEyeHeight(), 1.0f);
    ...
    private Vec3 getPosition(LivingEntity entity, double yOffset, float partialTicks) {
        double sx = Mth.lerp((double)partialTicks, (double)entity.xOld, (double)entity.getX());
        double sy = Mth.lerp((double)partialTicks, (double)entity.yOld, (double)entity.getY()) + yOffset;
        double sz = Mth.lerp((double)partialTicks, (double)entity.zOld, (double)entity.getZ());
    ...
    private static void renderBeam(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, Vec3 beamVector, float timeInTicks, float scale, float texVOff) {
        ...
        submitNodeCollector.submitCustomGeometry(poseStack, BEAM_RENDER_TYPE, (pose, buffer) -> {
            ...
            float vBase = Mth.floor((float)timeInTicks) % 2 == 0 ? 0.5f : 0.0f;
