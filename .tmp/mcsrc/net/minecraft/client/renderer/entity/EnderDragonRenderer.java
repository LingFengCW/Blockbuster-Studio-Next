...
extends EntityRenderer<EnderDragon, EnderDragonRenderState> {
    public static final Identifier CRYSTAL_BEAM_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/end_crystal/end_crystal_beam.png");
    private static final Identifier DRAGON_EXPLODING_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/enderdragon/dragon_exploding.png");
    private static final Identifier DRAGON_TEXTURE_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/enderdragon/dragon.png");
    private static final Identifier DRAGON_EYES_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/enderdragon/dragon_eyes.png");
    ...
    public void submit(EnderDragonRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera) {
        ...
        if (state.deathTime > 0.0f) {
            int color = ARGB.white((float)(1.0f - state.deathTime / 200.0f));
    ...
    private static void submitRays(PoseStack poseStack, float deathTime, SubmitNodeCollector submitNodeCollector, RenderType renderType) {
        submitNodeCollector.submitCustomGeometry(poseStack, renderType, (pose, buffer) -> {
            ...
            int innerColor = ARGB.colorFromFloat((float)(1.0f - overDrive), (float)1.0f, (float)1.0f, (float)1.0f);
            int outerColor = 0xFF00FF;
            RandomSource random = RandomSource.createThreadLocalInstance((long)432L);
            ...
            int rayCount = Mth.floor((float)((deathTime + deathTime * deathTime) / 2.0f * 60.0f));
    ...
    public static void submitCrystalBeams(float deltaX, float deltaY, float deltaZ, float timeInTicks, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords) {
        float horizontalLength = Mth.sqrt((float)(deltaX * deltaX + deltaZ * deltaZ));
    ...
    public void extractRenderState(EnderDragon entity, EnderDragonRenderState state, float partialTicks) {
        ...
        state.flapTime = Mth.lerp((float)partialTicks, (float)entity.oFlapTime, (float)entity.flapTime);
        ...
        BlockPos egg = entity.level().getHeightmapPos(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, EndPodiumFeature.getLocation((BlockPos)entity.getFightOrigin()));
        state.distanceToEgg = egg.distToCenterSqr((Position)entity.position());
