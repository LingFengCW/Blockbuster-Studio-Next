...
extends LivingEntityRenderer<AvatarlikeEntity, AvatarRenderState, PlayerModel> {
    ...
    private static HumanoidModel.ArmPose getArmPose(Avatar avatar, ItemStack itemInHand, InteractionHand hand) {
        ...
        if (!avatar.swinging && itemInHand.is((Object)Items.CROSSBOW) && CrossbowItem.isCharged((ItemStack)itemInHand)) {
        ...
        if ((attack = (SwingAnimation)itemInHand.get(DataComponents.SWING_ANIMATION)) != null && attack.type() == SwingAnimationType.STAB && avatar.swinging) {
            return HumanoidModel.ArmPose.SPEAR;
        }
    ...
    public void extractRenderState(AvatarlikeEntity entity, AvatarRenderState state, float partialTicks) {
        ...
        if (state.isUsingItem && (useItem = entity.getItemInHand(state.useItemHand)).is((Object)Items.SPYGLASS)) {
    ...
    private void extractCapeState(AvatarlikeEntity entity, AvatarRenderState state, float partialTicks) {
        ...
        double deltaX = clientState.getInterpolatedCloakX(partialTicks) - Mth.lerp((double)partialTicks, (double)((Avatar)entity).xo, (double)entity.getX());
        double deltaY = clientState.getInterpolatedCloakY(partialTicks) - Mth.lerp((double)partialTicks, (double)((Avatar)entity).yo, (double)entity.getY());
        double deltaZ = clientState.getInterpolatedCloakZ(partialTicks) - Mth.lerp((double)partialTicks, (double)((Avatar)entity).zo, (double)entity.getZ());
        float yBodyRot = Mth.rotLerp((float)partialTicks, (float)((Avatar)entity).yBodyRotO, (float)((Avatar)entity).yBodyRot);
        double forwardX = Mth.sin((double)(yBodyRot * ((float)Math.PI / 180)));
        double forwardZ = -Mth.cos((double)(yBodyRot * ((float)Math.PI / 180)));
        state.capeFlap = (float)deltaY * 10.0f;
        state.capeFlap = Mth.clamp((float)state.capeFlap, (float)-6.0f, (float)32.0f);
        state.capeLean = (float)(deltaX * forwardX + deltaZ * forwardZ) * 100.0f;
        state.capeLean *= 1.0f - state.fallFlyingScale();
        state.capeLean = Mth.clamp((float)state.capeLean, (float)0.0f, (float)150.0f);
        state.capeLean2 = (float)(deltaX * forwardZ - deltaZ * forwardX) * 100.0f;
        state.capeLean2 = Mth.clamp((float)state.capeLean2, (float)-20.0f, (float)20.0f);
    ...
    protected void setupRotations(AvatarRenderState state, PoseStack poseStack, float bodyRot, float entityScale) {
        ...
        if (state.isFallFlying) {
            ...
            float xAngle = Mth.lerp((float)swimAmount, (float)0.0f, (float)targetXRot);
