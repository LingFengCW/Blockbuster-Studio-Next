...
implements DebugRenderer.SimpleDebugRenderer {
        ...
        if (this.data != null) {
            ...
            for (Map.Entry<ChunkPos, String> entry : this.data.clientData.entrySet()) {
                ...
                for (String part : parts) {
                    Gizmos.billboardText((String)part, (Vec3)new Vec3((double)SectionPos.sectionToBlockCoord((int)pos.x(), (int)8), y + (double)yOffset, (double)SectionPos.sectionToBlockCoord((int)pos.z(), (int)8)), (TextGizmo.Style)TextGizmo.Style.whiteAndCentered().withScale(2.4f)).setAlwaysOnTop();
                    yOffset -= 2;
                }
            }
        }
    ...
    private final class ChunkData {
        ...
        private ChunkData(ChunkDebugRenderer chunkDebugRenderer, IntegratedServer server, double camX, double camZ) {
            ...
            ResourceKey dimension = clientLevel.dimension();
            int cx = SectionPos.posToSectionCoord((double)camX);
            int cz = SectionPos.posToSectionCoord((double)camZ);
