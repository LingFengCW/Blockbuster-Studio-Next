...
implements DebugRenderer.SimpleDebugRenderer {
    ...
    public void emitGizmos(double camX, double camY, double camZ, DebugValueAccess debugValues, Frustum frustum, float partialTicks) {
        ...
        if (time - this.lastUpdateTime > 1.0E8) {
            ...
            this.surroundEntities = ImmutableList.copyOf((Collection)cameraEntity.level().getEntities(cameraEntity, cameraEntity.getBoundingBox().inflate(16.0)));
        }
        if ((player = this.minecraft.player) != null && player.mainSupportingBlockPos.isPresent()) {
            this.drawHighlights((Entity)player, () -> 0.0, -65536);
        }
    ...
    private void drawHighlights(Entity entity, DoubleSupplier biasGetter, int color) {
        entity.mainSupportingBlockPos.ifPresent(bp -> {
            ...
            if (!effect.equals((Object)supportingBlock)) {
    ...
    private void highlightPosition(BlockPos pos, double offset, int color) {
        ...
        Gizmos.cuboid((AABB)new AABB(fromX, fromY, fromZ, toX, toY, toZ), (GizmoStyle)GizmoStyle.stroke((int)ARGB.color((float)0.4f, (int)color)));
        VoxelShape shape = this.minecraft.level.getBlockState(pos).getCollisionShape((BlockGetter)this.minecraft.level, pos, CollisionContext.empty()).move((Vec3i)pos);
        GizmoStyle style = GizmoStyle.stroke((int)color);
        for (AABB aabb : shape.toAabbs()) {
            Gizmos.cuboid((AABB)aabb, (GizmoStyle)style);
        }
    }
}
