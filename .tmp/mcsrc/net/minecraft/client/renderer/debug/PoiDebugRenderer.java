...
implements DebugRenderer.SimpleDebugRenderer {
    ...
    private static void highlightPoi(BlockPos poiPos) {
        float padding = 0.05f;
        Gizmos.cuboid((BlockPos)poiPos, (float)0.05f, (GizmoStyle)GizmoStyle.fill((int)ARGB.colorFromFloat((float)0.3f, (float)0.2f, (float)0.2f, (float)1.0f)));
    }

    private void renderGhostPoi(BlockPos poiPos, List<String> names) {
        float padding = 0.05f;
        Gizmos.cuboid((BlockPos)poiPos, (float)0.05f, (GizmoStyle)GizmoStyle.fill((int)ARGB.colorFromFloat((float)0.3f, (float)0.2f, (float)0.2f, (float)1.0f)));
        Gizmos.billboardTextOverBlock((String)names.toString(), (BlockPos)poiPos, (int)0, (int)-256, (float)0.32f);
        Gizmos.billboardTextOverBlock((String)"Ghost POI", (BlockPos)poiPos, (int)1, (int)-65536, (float)0.32f);
    }
    ...
    private static void renderTextOverPoi(String text, DebugPoiInfo poi, int row, int color) {
        Gizmos.billboardTextOverBlock((String)text, (BlockPos)poi.pos(), (int)row, (int)color, (float)0.32f);
    }
    ...
    private List<String> getTicketHolderNames(DebugPoiInfo poi, boolean potential, DebugValueAccess debugValues) {
        ...
        debugValues.forEachEntity(DebugSubscriptions.BRAINS, (entity, brainDump) -> {
            ...
            if (include) {
                names.add(DebugEntityNameGenerator.getEntityName((UUID)entity.getUUID()));
            }
        });
        return names;
    }
