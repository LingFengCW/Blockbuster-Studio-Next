...
implements DebugRenderer.SimpleDebugRenderer {
    ...
    public void emitGizmos(double camX, double camY, double camZ, DebugValueAccess debugValues, Frustum frustum, float partialTicks) {
        ...
        for (Entity entity : this.minecraft.level.entitiesForRendering()) {
            ...
            Gizmos.billboardText((String)"Missing Server Entity", (Vec3)entity.getPosition(entityPartialTicks).add(0.0, entity.getBoundingBox().getYsize() + 1.5, 0.0), (TextGizmo.Style)TextGizmo.Style.forColorAndCentered((int)-65536));
        }
    }
    ...
    private void showHitboxes(Entity entity, float partialTicks, boolean isServerEntity) {
        ...
        Gizmos.cuboid((AABB)entity.getBoundingBox().move(offset), (GizmoStyle)GizmoStyle.stroke((int)mainColor));
        Gizmos.point((Vec3)currentPosition, (int)mainColor, (float)2.0f);
        ...
        if (vehicle != null) {
            ...
            Gizmos.cuboid((AABB)new AABB(position.x - (double)width, position.y, position.z - (double)width, position.x + (double)width, position.y + 0.0625, position.z + (double)width), (GizmoStyle)GizmoStyle.stroke((int)-256));
        }
        if (entity instanceof LivingEntity) {
            AABB bb = entity.getBoundingBox().move(offset);
            float padding = 0.01f;
            Gizmos.cuboid((AABB)new AABB(bb.minX, bb.minY + (double)entity.getEyeHeight() - (double)0.01f, bb.minZ, bb.maxX, bb.minY + (double)entity.getEyeHeight() + (double)0.01f, bb.maxZ), (GizmoStyle)GizmoStyle.stroke((int)-65536));
        }
        if (entity instanceof EnderDragon) {
            ...
            for (EnderDragonPart subEntity : dragon.getSubEntities()) {
                ...
                Gizmos.cuboid((AABB)subEntity.getBoundingBox().move(subOffset), (GizmoStyle)GizmoStyle.stroke((int)ARGB.colorFromFloat((float)1.0f, (float)0.25f, (float)1.0f, (float)0.0f)));
            }
        }
        Vec3 eyePosition = currentPosition.add(0.0, (double)entity.getEyeHeight(), 0.0);
        Vec3 viewVector = entity.getViewVector(partialTicks);
        Gizmos.arrow((Vec3)eyePosition, (Vec3)eyePosition.add(viewVector.scale(2.0)), (int)-16776961);
        if (isServerEntity) {
            Vec3 deltaMovement = entity.getDeltaMovement();
            Gizmos.arrow((Vec3)currentPosition, (Vec3)currentPosition.add(deltaMovement), (int)-256);
        }
    }
}
