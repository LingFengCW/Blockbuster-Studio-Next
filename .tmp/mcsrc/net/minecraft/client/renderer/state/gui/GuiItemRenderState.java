...
implements ScreenArea {
    ...
    private @Nullable ScreenRectangle calculateOversizedItemBounds() {
        ...
        int actualXSize = Mth.ceil((double)(aabb.getXsize() * 16.0));
        int actualYSize = Mth.ceil((double)(aabb.getYsize() * 16.0));
        if (actualXSize > 16 || actualYSize > 16) {
            float xOffset = (float)(aabb.minX * 16.0);
            float yOffset = (float)(aabb.maxY * 16.0);
            int flooredXOffset = Mth.floor((float)xOffset);
            int flooredYOffset = Mth.floor((float)yOffset);
