...
public class ItemInHandRenderer {
    private static final RenderType MAP_BACKGROUND = RenderTypes.text(Identifier.withDefaultNamespace((String)"textures/map/map_background.png"));
    private static final RenderType MAP_BACKGROUND_CHECKERBOARD = RenderTypes.text(Identifier.withDefaultNamespace((String)"textures/map/map_background_checkerboard.png"));
    ...
    public void renderItem(LivingEntity mob, ItemStack itemStack, ItemDisplayContext type, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords) {
        ...
        this.itemModelResolver.updateForTopItem(renderState, itemStack, type, mob.level(), (ItemOwner)mob, mob.getId() + type.ordinal());
    ...
    private float calculateMapTilt(float xRot) {
        ...
        tilt = Mth.clamp((float)tilt, (float)0.0f, (float)1.0f);
    ...
    private void renderOneHandedMap(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, float inverseArmHeight, HumanoidArm arm, float attackValue, ItemStack map) {
        ...
        float sqrtAttackValue = Mth.sqrt((float)attackValue);
    ...
    private void renderMap(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, ItemStack itemStack) {
        ...
        MapItemSavedData data = MapItem.getSavedData((MapId)id, (Level)this.minecraft.level);
    ...
    private void applyEatTransform(PoseStack poseStack, float frameInterp, HumanoidArm arm, ItemStack itemStack, Player player) {
        ...
        float scaledUsageTime = currUsageTime / (float)itemStack.getUseDuration((LivingEntity)player);
        if (scaledUsageTime < 0.8f) {
            float extraHeightOffset = Mth.abs((float)(Mth.cos((double)(currUsageTime / 4.0f * (float)Math.PI)) * 0.1f));
    ...
    public void submitHandsWithItems(float frameInterp, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, LocalPlayer player, int lightCoords) {
        ...
        InteractionHand attackHand = (InteractionHand)MoreObjects.firstNonNull((Object)player.swingingArm, (Object)InteractionHand.MAIN_HAND);
        ...
        float xBob = Mth.lerp((float)frameInterp, (float)player.xBobO, (float)player.xBob);
        float yBob = Mth.lerp((float)frameInterp, (float)player.yBobO, (float)player.yBob);
        ...
        if (handRenderSelection.renderMainHand) {
            ...
            float mainhandInverseArmHeight = this.itemModelResolver.swapAnimationScale(this.mainHandItem) * (1.0f - Mth.lerp((float)frameInterp, (float)this.oMainHandHeight, (float)this.mainHandHeight));
        ...
        if (handRenderSelection.renderOffHand) {
            ...
            float offhandInverseArmHeight = this.itemModelResolver.swapAnimationScale(this.offHandItem) * (1.0f - Mth.lerp((float)frameInterp, (float)this.oOffHandHeight, (float)this.offHandHeight));
    ...
    static HandRenderSelection evaluateWhichHandsToRender(LocalPlayer player) {
        ...
        boolean holdsBow = mainHandItem.is((Object)Items.BOW) || offhandItem.is((Object)Items.BOW);
        boolean bl = holdsCrossbow = mainHandItem.is((Object)Items.CROSSBOW) || offhandItem.is((Object)Items.CROSSBOW);
    ...
    private static HandRenderSelection selectionUsingItemWhileHoldingBowLike(LocalPlayer player) {
        ...
        if (usedItemStack.is((Object)Items.BOW) || usedItemStack.is((Object)Items.CROSSBOW)) {
    ...
    private static boolean isChargedCrossbow(ItemStack item) {
        return item.is((Object)Items.CROSSBOW) && CrossbowItem.isCharged((ItemStack)item);
    }
    ...
    private void submitArmWithItem(AbstractClientPlayer player, float frameInterp, float xRot, InteractionHand hand, float attack, ItemStack itemStack, float inverseArmHeight, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords) {
        ...
        if (itemStack.isEmpty()) {
        ...
        } else if (itemStack.is((Object)Items.CROSSBOW)) {
            int invert;
            this.applyItemArmTransform(poseStack, arm, inverseArmHeight);
            boolean charged = CrossbowItem.isCharged((ItemStack)itemStack);
            ...
            if (player.isUsingItem() && player.getUseItemRemainingTicks() > 0 && player.getUsedItemHand() == hand && !charged) {
                ...
                float timeHeld = (float)itemStack.getUseDuration((LivingEntity)player) - ((float)player.getUseItemRemainingTicks() - frameInterp + 1.0f);
                float power = timeHeld / (float)CrossbowItem.getChargeDuration((ItemStack)itemStack, (LivingEntity)player);
                if (power > 1.0f) {
                    power = 1.0f;
                }
                if (power > 0.1f) {
            ...
            this.renderItem((LivingEntity)player, itemStack, isRightArm ? ItemDisplayContext.FIRST_PERSON_RIGHT_HAND : ItemDisplayContext.FIRST_PERSON_LEFT_HAND, poseStack, submitNodeCollector, lightCoords);
        } else {
            int invert;
            boolean isRightArm = arm == HumanoidArm.RIGHT;
            int n = invert = isRightArm ? 1 : -1;
    ...
    private void swingArm(float attack, PoseStack poseStack, int invert, HumanoidArm arm) {
        float xSwingPosition = -0.4f * Mth.sin((double)(Mth.sqrt((float)attack) * (float)Math.PI));
        float ySwingPosition = 0.2f * Mth.sin((double)(Mth.sqrt((float)attack) * ((float)Math.PI * 2)));
    ...
    private boolean shouldInstantlyReplaceVisibleItem(ItemStack currentlyVisibleItem, ItemStack expectedItem) {
        if (ItemStack.matchesIgnoringComponents((ItemStack)currentlyVisibleItem, (ItemStack)expectedItem, DataComponentType::ignoreSwapAnimation)) {
            return true;
        }
    ...
    public void tick() {
        ...
        if (player.isHandsBusy()) {
            ...
            this.mainHandHeight += Mth.clamp((float)(mainHandTargetHeight - this.mainHandHeight), (float)-0.4f, (float)0.4f);
            this.offHandHeight += Mth.clamp((float)(offHandTargetHeight - this.offHandHeight), (float)-0.4f, (float)0.4f);
        }
