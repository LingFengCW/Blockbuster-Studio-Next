...
implements AutoCloseable {
    public static final Identifier FORCEFIELD_LOCATION = Identifier.withDefaultNamespace((String)"textures/misc/forcefield.png");
    ...
    private void rebuildWorldBorderBuffer(WorldBorderRenderState state, double renderDistance, double cameraZ, double cameraX, float halfHeightY, float v1, float v0) {
        try (ByteBufferBuilder byteBufferBuilder = ByteBufferBuilder.exactlySized(DefaultVertexFormat.POSITION_TEX.getVertexSize() * 4 * 4);){
            ...
            double minZ = Math.max((double)Mth.floor((double)(cameraZ - renderDistance)), borderMinZ);
            double maxZ = Math.min((double)Mth.ceil((double)(cameraZ + renderDistance)), borderMaxZ);
            float u0z = (float)(Mth.floor((double)minZ) & 1) * 0.5f;
            float u1z = (float)(maxZ - minZ) / 2.0f;
            double minX = Math.max((double)Mth.floor((double)(cameraX - renderDistance)), borderMinX);
            double maxX = Math.min((double)Mth.ceil((double)(cameraX + renderDistance)), borderMaxX);
            float u0x = (float)(Mth.floor((double)minX) & 1) * 0.5f;
    ...
    public void extract(WorldBorder border, float deltaPartialTick, Vec3 cameraPos, double renderDistance, WorldBorderRenderState state) {
        ...
        state.alpha = Mth.clamp((double)state.alpha, (double)0.0, (double)1.0);
    ...
    public void render(WorldBorderRenderState state, Vec3 cameraPos, double renderDistance, double depthFar) {
        ...
        float red = (float)ARGB.red((int)state.tint) / 255.0f;
        float green = (float)ARGB.green((int)state.tint) / 255.0f;
        float blue = (float)ARGB.blue((int)state.tint) / 255.0f;
        float offset = (float)(Util.getMillis() % 3000L) / 3000.0f;
        float v0 = (float)(-Mth.frac((double)(cameraPos.y * 0.5)));
