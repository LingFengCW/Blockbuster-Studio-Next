    ...
    private Preparations stitch(List<SpriteContents> sprites, int maxMipmapLevels, Executor executor) {
        try (Zone ignored = Profiler.get().zone(() -> "stitch " + String.valueOf(this.location));){
            ...
            for (SpriteContents spriteInfo : sprites) {
                ...
                LOGGER.warn("Texture {} with size {}x{} limits mip level from {} to {}", new Object[]{spriteInfo.name(), spriteInfo.width(), spriteInfo.height(), Mth.log2((int)lowestOneBit), Mth.log2((int)lowestTextureBit)});
                lowestOneBit = lowestTextureBit;
            }
            int minSize = Math.min(minTexelSize, lowestOneBit);
            int minPowerOfTwo = Mth.log2((int)minSize);
            ...
            try {
                ...
                CrashReport report = CrashReport.forThrowable((Throwable)e, (String)"Stitching");
                CrashReportCategory category = report.addCategory("Stitcher");
                category.setDetail("Sprites", (Object)e.getAllSprites().stream().map(s -> String.format(Locale.ROOT, "%s[%dx%d]", s.name(), s.width(), s.height())).collect(Collectors.joining(",")));
