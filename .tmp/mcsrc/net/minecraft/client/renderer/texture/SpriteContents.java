...
Stitcher.Entry {
    ...
    public void increaseMipLevel(int mipLevel) {
        try {
            ...
            CrashReport report = CrashReport.forThrowable((Throwable)t, (String)"Generating mipmaps for frame");
    ...
    public <T> Optional<T> getAdditionalMetadata(MetadataSectionType<T> type) {
        for (MetadataSectionType.WithValue<?> metadata : this.additionalMetadata) {
            Optional result = metadata.unwrapToType(type);
    ...
    public boolean isTransparent(int frame, int x, int y) {
        ...
        return ARGB.alpha((int)this.originalImage.getPixel(actualX, actualY)) == 0;
    }
    ...
    public Transparency computeTransparency(float u0, float v0, float u1, float v1) {
        ...
        int x0 = Mth.floor((float)(u0 * (float)this.width));
        int y0 = Mth.floor((float)(v0 * (float)this.height));
        int x1 = Mth.ceil((float)(u1 * (float)this.width));
        int y1 = Mth.ceil((float)(v1 * (float)this.height));
        if (this.animatedTexture != null) {
