...
public record PalettedPermutations(List<Identifier> textures, Identifier paletteKey, Map<String, Identifier> permutations, String separator) implements SpriteSource
{
    ...
    public static final MapCodec<PalettedPermutations> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group((App)Codec.list((Codec)Identifier.CODEC).fieldOf("textures").forGetter(PalettedPermutations::textures), (App)Identifier.CODEC.fieldOf("palette_key").forGetter(PalettedPermutations::paletteKey), (App)Codec.unboundedMap((Codec)Codec.STRING, (Codec)Identifier.CODEC).fieldOf("permutations").forGetter(PalettedPermutations::permutations), (App)Codec.STRING.optionalFieldOf("separator", (Object)DEFAULT_SEPARATOR).forGetter(PalettedPermutations::separator)).apply((Applicative)i, PalettedPermutations::new));
    ...
    public void run(ResourceManager resourceManager, SpriteSource.Output output) {
        ...
        for (Identifier textureLocation : this.textures) {
            ...
            Optional resource = resourceManager.getResource(textureId);
            if (resource.isEmpty()) {
    ...
    private static IntUnaryOperator createPaletteMapping(int[] keys, int[] values) {
        ...
        for (int i = 0; i < keys.length; ++i) {
            int key = keys[i];
            if (ARGB.alpha((int)key) == 0) continue;
            palette.put(ARGB.transparent((int)key), values[i]);
        }
    ...
    private static int[] loadPaletteEntryFromImage(ResourceManager resourceManager, Identifier location) {
        Optional resource = resourceManager.getResource(TEXTURE_ID_CONVERTER.idToFile(location));
        if (resource.isEmpty()) {
    ...
    private static /* synthetic */ int lambda$createPaletteMapping$0(Int2IntMap palette, int pixel) {
        int pixelAlpha = ARGB.alpha((int)pixel);
        if (pixelAlpha == 0) {
            return pixel;
        }
        int pixelRGB = ARGB.transparent((int)pixel);
        int value = palette.getOrDefault(pixelRGB, ARGB.opaque((int)pixelRGB));
        int valueAlpha = ARGB.alpha((int)value);
        return ARGB.color((int)(pixelAlpha * valueAlpha / 255), (int)value);
    }
