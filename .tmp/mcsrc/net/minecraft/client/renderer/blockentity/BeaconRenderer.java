 ...
 *  net.minecraft.world.level.block.entity.BeaconBeamOwner
...
implements BlockEntityRenderer<T, BeaconRenderState> {
    public static final Identifier BEAM_LOCATION = Identifier.withDefaultNamespace((String)"textures/entity/beacon/beacon_beam.png");
    ...
    public static <T extends BlockEntity> void extract(T blockEntity, BeaconRenderState state, float partialTicks, Vec3 cameraPosition) {
        ...
        float distanceToBeacon = (float)cameraPosition.subtract(Vec3.atCenterOf((Vec3i)state.blockPos)).horizontalDistance();
    ...
    public static void submitBeaconBeam(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, Identifier beamLocation, float scale, float animationTime, int beamStart, int height, int color, float solidBeamRadius, float beamGlowRadius) {
        ...
        float texVOff = Mth.frac((float)(scroll * 0.2f - (float)Mth.floor((float)(scroll * 0.1f))));
        poseStack.pushPose();
        ...
        submitNodeCollector.submitCustomGeometry(poseStack, RenderTypes.beaconBeam(beamLocation, true), (pose, buffer) -> BeaconRenderer.renderPart(pose, buffer, ARGB.color((int)32, (int)color), beamStart, beamEnd, wnx, wnz, enx, enz, wsx, wsz, esx, esz, 0.0f, 1.0f, vv1, vv2));
        poseStack.popPose();
    }
    ...
    public boolean shouldRender(T blockEntity, Vec3 cameraPosition) {
        return Vec3.atCenterOf((Vec3i)blockEntity.getBlockPos()).multiply(1.0, 0.0, 1.0).closerThan((Position)cameraPosition.multiply(1.0, 0.0, 1.0), (double)this.getViewDistance());
    }
}
