...
implements BlockEntityRenderer<ShelfBlockEntity, ShelfRenderState> {
    ...
    public void extractRenderState(ShelfBlockEntity blockEntity, ShelfRenderState state, float partialTicks, Vec3 cameraPosition, @Nullable ModelFeatureRenderer.CrumblingOverlay breakProgress) {
        ...
        state.facing = (Direction)blockEntity.getBlockState().getValue((Property)ShelfBlock.FACING);
        NonNullList items = blockEntity.getItems();
        ...
        for (int slot = 0; slot < items.size(); ++slot) {
            ...
            this.itemModelResolver.updateForTopItem(itemStackRenderState, itemStack, ItemDisplayContext.ON_SHELF, blockEntity.level(), (ItemOwner)blockEntity, seed + slot);
