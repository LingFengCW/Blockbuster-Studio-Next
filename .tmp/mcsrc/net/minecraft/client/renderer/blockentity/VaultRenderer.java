...
implements BlockEntityRenderer<VaultBlockEntity, VaultRenderState> {
    ...
    public void extractRenderState(VaultBlockEntity blockEntity, VaultRenderState state, float partialTicks, Vec3 cameraPosition, @Nullable ModelFeatureRenderer.CrumblingOverlay breakProgress) {
        ...
        if (!VaultBlockEntity.Client.shouldDisplayActiveEffects((VaultSharedData)blockEntity.getSharedData()) || displayItem.isEmpty() || blockEntity.getLevel() == null) {
            return;
        }
        ...
        state.spin = Mth.rotLerp((float)partialTicks, (float)clientData.previousSpin(), (float)clientData.currentSpin());
    }
