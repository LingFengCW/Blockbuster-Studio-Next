...
implements BlockEntityRenderer<EnchantingTableBlockEntity, EnchantTableRenderState> {
    ...
    public void extractRenderState(EnchantingTableBlockEntity blockEntity, EnchantTableRenderState state, float partialTicks, Vec3 cameraPosition, @Nullable ModelFeatureRenderer.CrumblingOverlay breakProgress) {
        ...
        state.flip = Mth.lerp((float)partialTicks, (float)blockEntity.oFlip, (float)blockEntity.flip);
        state.open = Mth.lerp((float)partialTicks, (float)blockEntity.oOpen, (float)blockEntity.open);
    ...
    public void submit(EnchantTableRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera) {
        ...
        float ff1 = Mth.frac((float)(state.flip + 0.25f)) * 1.6f - 0.3f;
        float ff2 = Mth.frac((float)(state.flip + 0.75f)) * 1.6f - 0.3f;
        BookModel.State bookState = BookModel.State.forAnimation(state.time, Mth.clamp((float)ff1, (float)0.0f, (float)1.0f), Mth.clamp((float)ff2, (float)0.0f, (float)1.0f), state.open);
