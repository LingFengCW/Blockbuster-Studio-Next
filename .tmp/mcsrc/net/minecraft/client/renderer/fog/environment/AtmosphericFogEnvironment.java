...
extends FogEnvironment {
    ...
    public int getBaseColor(ClientLevel level, Camera camera, int renderDistance, float partialTicks) {
        ...
        if (renderDistance >= 4) {
            int color;
            float alpha;
            float sunAngle = ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.SUN_ANGLE, partialTicks)).floatValue() * ((float)Math.PI / 180);
            ...
            if (lookingAtTheSunFactor > 0.0f && (alpha = ARGB.alphaFloat((int)(color = ((Integer)camera.attributeProbe().getValue(EnvironmentAttributes.SUNRISE_SUNSET_COLOR, partialTicks)).intValue()))) > 0.0f) {
                fogColor = ARGB.srgbLerp((float)(lookingAtTheSunFactor * alpha), (int)fogColor, (int)ARGB.opaque((int)color));
            }
        }
        ...
        float skyFogEnd = Math.min(((Float)camera.attributeProbe().getValue(EnvironmentAttributes.SKY_FOG_END_DISTANCE, partialTicks)).floatValue() / 16.0f, (float)renderDistance);
        float skyColorMixFactor = Mth.clampedLerp((float)(skyFogEnd / 32.0f), (float)0.25f, (float)1.0f);
        skyColorMixFactor = 1.0f - (float)Math.pow(skyColorMixFactor, 0.25);
        fogColor = ARGB.srgbLerp((float)skyColorMixFactor, (int)fogColor, (int)skyColor);
        return fogColor;
    }
    ...
    private static int applyWeatherDarken(int color, float rainLevel, float thunderLevel) {
        if (rainLevel > 0.0f) {
            ...
            color = ARGB.scaleRGB((int)color, (float)rainColorModifier, (float)rainColorModifier, (float)rainBlueColorModifier);
        }
        if (thunderLevel > 0.0f) {
            color = ARGB.scaleRGB((int)color, (float)(1.0f - thunderLevel * 0.5f));
        }
        return color;
    }
    ...
    public void setupFog(FogData fog, Camera camera, ClientLevel level, float renderDistance, DeltaTracker deltaTracker) {
        ...
        fog.environmentalStart = ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.FOG_START_DISTANCE, partialTicks)).floatValue();
        fog.environmentalEnd = ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.FOG_END_DISTANCE, partialTicks)).floatValue();
        ...
        fog.skyEnd = Math.min(renderDistance, ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.SKY_FOG_END_DISTANCE, partialTicks)).floatValue());
        fog.cloudEnd = Math.min((float)(Minecraft.getInstance().options.cloudRange().get() * 16), ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.CLOUD_FOG_END_DISTANCE, partialTicks)).floatValue());
    ...
    private void updateRainFogState(Camera camera, ClientLevel level, DeltaTracker deltaTracker) {
        ...
        float skyLightLevelMultiplier = Mth.clamp((float)(((float)level.getLightEngine().getLayerListener(LightLayer.SKY).getLightValue(blockPos) - 8.0f) / 7.0f), (float)0.0f, (float)1.0f);
