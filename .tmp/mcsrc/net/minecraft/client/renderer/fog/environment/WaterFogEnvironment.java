...
extends FogEnvironment {
    ...
    public void setupFog(FogData fog, Camera camera, ClientLevel level, float renderDistance, DeltaTracker deltaTracker) {
        ...
        fog.environmentalStart = ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.WATER_FOG_START_DISTANCE, partialTicks)).floatValue();
        fog.environmentalEnd = ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.WATER_FOG_END_DISTANCE, partialTicks)).floatValue();
