...
implements AutoCloseable {
    ...
    private static final Identifier TEXTURE_LOCATION = Identifier.withDefaultNamespace((String)"textures/environment/clouds.png");
    ...
    private static boolean isCellEmpty(int color) {
        return ARGB.alpha((int)color) < 10;
    }
    ...
    public void render(int color, CloudStatus cloudStatus, float bottomY, int range, Vec3 cameraPosition, long gameTime, float partialTicks) {
        ...
        int radiusCells = Mth.ceil((float)((float)radiusBlocks / 12.0f));
        ...
        cloudX -= (double)Mth.floor((double)(cloudX / textureWidthBlocks)) * textureWidthBlocks;
        cloudZ -= (double)Mth.floor((double)(cloudZ / textureHeightBlocks)) * textureHeightBlocks;
        int cellX = Mth.floor((double)(cloudX / 12.0));
        int cellZ = Mth.floor((double)(cloudZ / 12.0));
        ...
        try {
            Std140Builder.intoBuffer(view.data()).putVec4((Vector4fc)ARGB.vector4fFromARGB32((int)color)).putVec3(-xInCell, relativeBottomY, -zInCell).putVec3(12.0f, 4.0f, 12.0f);
        }
        finally {
            if (view != null) {
                view.close();
