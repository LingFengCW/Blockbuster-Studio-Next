...
implements AutoCloseable {
    private static final Identifier TRANSPARENCY_POST_CHAIN_ID = Identifier.withDefaultNamespace((String)"transparency");
    private static final Identifier ENTITY_OUTLINE_POST_CHAIN_ID = Identifier.withDefaultNamespace((String)"entity_outline");
    ...
    public void render(GraphicsResourceAllocator resourceAllocator, DeltaTracker deltaTracker, boolean renderOutline, CameraRenderState cameraState, Matrix4fc modelViewMatrix, GpuBufferSlice terrainFog, Vector4f fogColor, boolean shouldRenderSky) {
        ...
        if ((cloudStatus = this.optionsRenderState.cloudStatus) != CloudStatus.OFF && ARGB.alpha((int)this.levelRenderState.cloudColor) > 0) {
    ...
    private void compileSections(CameraRenderState camera) {
        ...
        long fadeDuration = Mth.floor((double)(this.optionsRenderState.chunkSectionFadeInTime * 1000.0));
        for (SectionUpdateRenderState state : this.levelRenderState.sectionUpdateRenderStates) {
            BlockPos center = SectionPos.of((long)state.sectionNode()).center();
            double distSqr = center.distSqr((Vec3i)cameraPosition);
    ...
    private void submitBlockOutline(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, LevelRenderState levelRenderState) {
        ...
        int outlineColor = state.highContrast() ? -11010079 : ARGB.black((int)102);
    ...
    private void submitHitOutline(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, RenderType renderType, BlockOutlineRenderState state, int color, float width, boolean afterTerrain) {
        if (SharedConstants.DEBUG_SHAPES) {
            ...
            if (state.collisionShape() != null) {
                submitNodeCollector.submitShapeOutline(poseStack, state.collisionShape(), renderType, ARGB.colorFromFloat((float)0.4f, (float)0.0f, (float)0.0f, (float)0.0f), width, afterTerrain);
            }
            if (state.occlusionShape() != null) {
                submitNodeCollector.submitShapeOutline(poseStack, state.occlusionShape(), renderType, ARGB.colorFromFloat((float)0.4f, (float)0.0f, (float)1.0f, (float)0.0f), width, afterTerrain);
            }
            if (state.interactionShape() != null) {
                submitNodeCollector.submitShapeOutline(poseStack, state.interactionShape(), renderType, ARGB.colorFromFloat((float)0.4f, (float)0.0f, (float)0.0f, (float)1.0f), width, afterTerrain);
            }
        } else {
    ...
    private void scheduleTranslucentSectionResort(Vec3 cameraPos) {
        ...
        boolean blockPosChanged = !cameraBlockPos.equals((Object)this.lastTranslucentSortBlockPos);
    ...
    public Gizmos.TemporaryCollection collectPerFrameRenderThreadGizmos() {
        return Gizmos.withCollector((GizmoCollector)this.renderThreadGizmos);
    }
    ...
    private void finalizeGizmoCollection() {
        ...
        for (SimpleGizmoCollector.GizmoInstance instance : this.renderThreadGizmos.drainGizmos()) {
            instance.gizmo().emit((GizmoPrimitives)(instance.isAlwaysOnTop() ? alwaysOnTopPrimitives : standardPrimitives), instance.getAlphaMultiplier(currentMillis));
        }
