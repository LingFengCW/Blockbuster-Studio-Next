...
public class EndFlashState {
    ...
    private void calculateFlashParameters(long clockTime) {
        ...
        if (newSeed != this.flashSeed) {
            RandomSource randomSource = RandomSource.createThreadLocalInstance((long)newSeed);
    ...
    private float calculateIntensity(long clockTime) {
        ...
        return Mth.sin((double)((float)(clockTimeWithinInterval - (long)this.offset) * (float)Math.PI / (float)this.duration));
    }
    ...
    public float getIntensity(float partialTicks) {
        return Mth.lerp((float)partialTicks, (float)this.oldIntensity, (float)this.intensity);
    }
