...
public interface Condition {
    public static final Codec<Condition> CODEC = Codec.recursive((String)"condition", self -> {
        Codec combinerCodec = Codec.simpleMap(CombinedCondition.Operation.CODEC, (Codec)self.listOf(), (Keyable)StringRepresentable.keys((StringRepresentable[])CombinedCondition.Operation.values())).codec().comapFlatMap(map -> {
            if (map.size() != 1) {
