...
public interface BlockStateModel {
    ...
    extends ResolvableModel {
        ...
        public static final Codec<WeightedVariants.Unbaked> HARDCODED_WEIGHTED_CODEC = ExtraCodecs.nonEmptyList((Codec)ELEMENT_CODEC.listOf()).flatComapMap(w -> new WeightedVariants.Unbaked((WeightedList<Unbaked>)WeightedList.of((List)Lists.transform((List)w, e -> e.map(SingleVariant.Unbaked::new)))), unbaked -> {
            List entries = unbaked.entries().unwrap();
            ArrayList<Weighted> result = new ArrayList<Weighted>(entries.size());
            for (Weighted entry : entries) {
                ...
                if (patt0$temp instanceof SingleVariant.Unbaked) {
                    ...
                    result.add(new Weighted((Object)singleVariant.variant(), entry.weight()));
                    continue;
                }
