...
public class ModelBlockRenderer {
    ...
    private final RandomSource random = RandomSource.createThreadLocalInstance((long)0L);
        ...
        for (BlockStateModelPart part : parts) {
            for (Direction direction : DIRECTIONS) {
                ...
                if (!validCacheForDirection) {
                    shouldRenderFace = this.shouldRenderFace(level, state, direction, (BlockPos)this.scratchPos.setWithOffset((Vec3i)pos, direction));
    ...
    private void tesselateFlat(BlockQuadOutput output, float x, float y, float z, List<BlockStateModelPart> parts, BlockAndTintGetter level, BlockState state, BlockPos pos) {
        ...
        for (BlockStateModelPart part : parts) {
            for (Direction direction : DIRECTIONS) {
                ...
                if (!validCacheForDirection) {
                    shouldRenderFace = this.shouldRenderFace(level, state, direction, (BlockPos)relativePos);
                    cacheValid |= cacheMask;
                    if (shouldRenderFace) {
                        shouldRenderFaceCache |= cacheMask;
                    }
                ...
                if (!shouldRenderFace) continue;
                int lightCoords = this.lighter.getLightCoords(state, level, (BlockPos)relativePos);
                for (BakedQuad quad : culledQuads) {
    ...
    private boolean shouldRenderFace(BlockAndTintGetter level, BlockState state, Direction direction, BlockPos neighborPos) {
        ...
        return Block.shouldRenderFace((BlockState)state, (BlockState)neighborState, (Direction)direction);
    }
