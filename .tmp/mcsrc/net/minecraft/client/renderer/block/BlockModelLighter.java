...
public class BlockModelLighter {
    ...
    public void prepareQuadAmbientOcclusion(BlockAndTintGetter level, BlockState state, BlockPos centerPosition, BakedQuad quad, QuadInstance outputInstance) {
        ...
        BlockState state0 = level.getBlockState((BlockPos)pos);
        int light0 = this.cache.getLightCoords(state0, level, (BlockPos)pos);
        float shade0 = this.cache.getShadeBrightness(state0, level, (BlockPos)pos);
        pos.setWithOffset((Vec3i)basePosition, info.corners[1]);
        BlockState state1 = level.getBlockState((BlockPos)pos);
        int light1 = this.cache.getLightCoords(state1, level, (BlockPos)pos);
        float shade1 = this.cache.getShadeBrightness(state1, level, (BlockPos)pos);
        pos.setWithOffset((Vec3i)basePosition, info.corners[2]);
        BlockState state2 = level.getBlockState((BlockPos)pos);
        int light2 = this.cache.getLightCoords(state2, level, (BlockPos)pos);
        float shade2 = this.cache.getShadeBrightness(state2, level, (BlockPos)pos);
        pos.setWithOffset((Vec3i)basePosition, info.corners[3]);
        BlockState state3 = level.getBlockState((BlockPos)pos);
        int light3 = this.cache.getLightCoords(state3, level, (BlockPos)pos);
        float shade3 = this.cache.getShadeBrightness(state3, level, (BlockPos)pos);
        BlockState corner0 = level.getBlockState((BlockPos)pos.setWithOffset((Vec3i)basePosition, info.corners[0]).move(direction));
        boolean translucent0 = !corner0.isViewBlocking((BlockGetter)level, (BlockPos)pos) || corner0.getLightDampening() == 0;
        BlockState corner1 = level.getBlockState((BlockPos)pos.setWithOffset((Vec3i)basePosition, info.corners[1]).move(direction));
        boolean translucent1 = !corner1.isViewBlocking((BlockGetter)level, (BlockPos)pos) || corner1.getLightDampening() == 0;
        BlockState corner2 = level.getBlockState((BlockPos)pos.setWithOffset((Vec3i)basePosition, info.corners[2]).move(direction));
        boolean translucent2 = !corner2.isViewBlocking((BlockGetter)level, (BlockPos)pos) || corner2.getLightDampening() == 0;
        BlockState corner3 = level.getBlockState((BlockPos)pos.setWithOffset((Vec3i)basePosition, info.corners[3]).move(direction));
        boolean bl = translucent3 = !corner3.isViewBlocking((BlockGetter)level, (BlockPos)pos) || corner3.getLightDampening() == 0;
        if (translucent2 || translucent0) {
            pos.setWithOffset((Vec3i)basePosition, info.corners[0]).move(info.corners[2]);
            BlockState state02 = level.getBlockState((BlockPos)pos);
            shadeCorner02 = this.cache.getShadeBrightness(state02, level, (BlockPos)pos);
            lightCorner02 = this.cache.getLightCoords(state02, level, (BlockPos)pos);
        } else {
        ...
        if (translucent3 || translucent0) {
            ...
            BlockState state03 = level.getBlockState((BlockPos)pos);
            shadeCorner03 = this.cache.getShadeBrightness(state03, level, (BlockPos)pos);
            lightCorner03 = this.cache.getLightCoords(state03, level, (BlockPos)pos);
        } else {
        ...
        if (translucent2 || translucent1) {
            ...
            BlockState state12 = level.getBlockState((BlockPos)pos);
            shadeCorner12 = this.cache.getShadeBrightness(state12, level, (BlockPos)pos);
            lightCorner12 = this.cache.getLightCoords(state12, level, (BlockPos)pos);
        } else {
        ...
        if (translucent3 || translucent1) {
            ...
            BlockState state13 = level.getBlockState((BlockPos)pos);
            shadeCorner13 = this.cache.getShadeBrightness(state13, level, (BlockPos)pos);
            lightCorner13 = this.cache.getLightCoords(state13, level, (BlockPos)pos);
        } else {
        ...
        BlockState nextState = level.getBlockState((BlockPos)pos);
        if (this.faceCubic || !nextState.isSolidRender()) {
            lightCenter = this.cache.getLightCoords(nextState, level, (BlockPos)pos);
        }
        ...
        if (!this.facePartial || !info.doNonCubicWeight) {
            ...
            outputInstance.setLightCoords(remap.vert0, LightCoordsUtil.smoothBlend((int)light3, (int)light0, (int)lightCorner03, (int)lightCenter));
            outputInstance.setLightCoords(remap.vert1, LightCoordsUtil.smoothBlend((int)light2, (int)light0, (int)lightCorner02, (int)lightCenter));
            outputInstance.setLightCoords(remap.vert2, LightCoordsUtil.smoothBlend((int)light2, (int)light1, (int)lightCorner12, (int)lightCenter));
            outputInstance.setLightCoords(remap.vert3, LightCoordsUtil.smoothBlend((int)light3, (int)light1, (int)lightCorner13, (int)lightCenter));
            outputInstance.setColor(remap.vert0, ARGB.gray((float)lightLevel1));
            outputInstance.setColor(remap.vert1, ARGB.gray((float)lightLevel2));
            outputInstance.setColor(remap.vert2, ARGB.gray((float)lightLevel3));
            outputInstance.setColor(remap.vert3, ARGB.gray((float)lightLevel4));
        } else {
            ...
            outputInstance.setColor(remap.vert0, ARGB.gray((float)Math.clamp(tempShade1 * vert0weight01 + tempShade2 * vert0weight23 + tempShade3 * vert0weight45 + tempShade4 * vert0weight67, 0.0f, 1.0f)));
            outputInstance.setColor(remap.vert1, ARGB.gray((float)Math.clamp(tempShade1 * vert1weight01 + tempShade2 * vert1weight23 + tempShade3 * vert1weight45 + tempShade4 * vert1weight67, 0.0f, 1.0f)));
            outputInstance.setColor(remap.vert2, ARGB.gray((float)Math.clamp(tempShade1 * vert2weight01 + tempShade2 * vert2weight23 + tempShade3 * vert2weight45 + tempShade4 * vert2weight67, 0.0f, 1.0f)));
            outputInstance.setColor(remap.vert3, ARGB.gray((float)Math.clamp(tempShade1 * vert3weight01 + tempShade2 * vert3weight23 + tempShade3 * vert3weight45 + tempShade4 * vert3weight67, 0.0f, 1.0f)));
            ...
            outputInstance.setLightCoords(remap.vert0, LightCoordsUtil.smoothWeightedBlend((int)_tc1, (int)_tc2, (int)_tc3, (int)_tc4, (float)vert0weight01, (float)vert0weight23, (float)vert0weight45, (float)vert0weight67));
            outputInstance.setLightCoords(remap.vert1, LightCoordsUtil.smoothWeightedBlend((int)_tc1, (int)_tc2, (int)_tc3, (int)_tc4, (float)vert1weight01, (float)vert1weight23, (float)vert1weight45, (float)vert1weight67));
            outputInstance.setLightCoords(remap.vert2, LightCoordsUtil.smoothWeightedBlend((int)_tc1, (int)_tc2, (int)_tc3, (int)_tc4, (float)vert2weight01, (float)vert2weight23, (float)vert2weight45, (float)vert2weight67));
            outputInstance.setLightCoords(remap.vert3, LightCoordsUtil.smoothWeightedBlend((int)_tc1, (int)_tc2, (int)_tc3, (int)_tc4, (float)vert3weight01, (float)vert3weight23, (float)vert3weight45, (float)vert3weight67));
        }
    ...
    public void prepareQuadFlat(BlockAndTintGetter level, BlockState state, BlockPos pos, int lightCoords, BakedQuad quad, QuadInstance outputInstance) {
        ...
        outputInstance.setColor(ARGB.gray((float)directionalBrightness));
    }
    ...
    private void prepareQuadShape(BlockAndTintGetter level, BlockState state, BlockPos pos, BakedQuad quad, boolean ambientOcclusion) {
        ...
        this.faceCubic = switch (quad.direction()) {
            default -> throw new MatchException(null, null);
            case Direction.DOWN -> {
                if (minY == maxY && (minY < 1.0E-4f || state.isCollisionShapeFullBlock((BlockGetter)level, pos))) {
                    yield true;
                }
                yield false;
            }
            case Direction.UP -> {
                if (minY == maxY && (maxY > 0.9999f || state.isCollisionShapeFullBlock((BlockGetter)level, pos))) {
                    yield true;
                }
                yield false;
            }
            case Direction.NORTH -> {
                if (minZ == maxZ && (minZ < 1.0E-4f || state.isCollisionShapeFullBlock((BlockGetter)level, pos))) {
                    yield true;
                }
                yield false;
            }
            case Direction.SOUTH -> {
                if (minZ == maxZ && (maxZ > 0.9999f || state.isCollisionShapeFullBlock((BlockGetter)level, pos))) {
                    yield true;
                }
                yield false;
            }
            case Direction.WEST -> {
                if (minX == maxX && (minX < 1.0E-4f || state.isCollisionShapeFullBlock((BlockGetter)level, pos))) {
                    yield true;
                }
                yield false;
            }
            case Direction.EAST -> minX == maxX && (maxX > 0.9999f || state.isCollisionShapeFullBlock((BlockGetter)level, pos));
        };
    }
    ...
    public static class Cache {
        ...
        public int getLightCoords(BlockState state, BlockAndTintGetter level, BlockPos pos) {
            return LightCoordsUtil.getLightCoords((LightCoordsUtil.BrightnessGetter)(this.enabled ? this.cachedBrightnessGetter : LightCoordsUtil.BrightnessGetter.DEFAULT), (BlockAndLightGetter)level, (BlockState)state, (BlockPos)pos);
        }
        ...
        public float getShadeBrightness(BlockState state, BlockAndTintGetter level, BlockPos pos) {
            ...
            float brightness = state.getShadeBrightness((BlockGetter)level, pos);
            if (this.enabled) {
    ...
    private static enum AdjacencyInfo {
        ...
        static {
            BY_FACING = (AdjacencyInfo[])Util.make((Object)new AdjacencyInfo[6], map -> {
    ...
    private static enum AmbientVertexRemap {
        ...
        static {
            BY_FACING = (AmbientVertexRemap[])Util.make((Object)new AmbientVertexRemap[6], map -> {
                map[Direction.DOWN.get3DDataValue()] = DOWN;
                map[Direction.UP.get3DDataValue()] = UP;
                map[Direction.NORTH.get3DDataValue()] = NORTH;
                map[Direction.SOUTH.get3DDataValue()] = SOUTH;
