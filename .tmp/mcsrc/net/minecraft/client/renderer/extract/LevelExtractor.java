...
implements ResourceManagerReloadListener {
    ...
    public void extract(DeltaTracker deltaTracker, Camera camera, float deltaPartialTick) {
        ...
        if (this.sectionUpdateTracker != null && this.level != null) {
            ...
            expectedChunks.forEach(expectedChunk -> {
                if (chunkCache.hasChunk(ChunkPos.getX((long)expectedChunk), ChunkPos.getZ((long)expectedChunk))) {
        ...
        if (ARGB.alpha((int)this.levelRenderState.cloudColor) > 0) {
            this.levelRenderState.cloudHeight = ((Float)camera.attributeProbe().getValue(EnvironmentAttributes.CLOUD_HEIGHT, deltaPartialTick)).floatValue();
        }
    ...
    private void extractVisibleEntities(Camera camera, Frustum frustum, DeltaTracker deltaTracker, LevelRenderState output) {
        ...
        Entity.setViewScale((double)(Mth.clamp((double)((double)this.minecraft.options.getEffectiveRenderDistance() / 8.0), (double)1.0, (double)2.5) * this.minecraft.options.entityDistanceScaling().get()));
    ...
    public boolean isEntityVisible(Entity entity, Frustum frustum, double camX, double camY, double camZ) {
        ...
        if (!(this.levelRenderer.entityRenderDispatcher().shouldRender(entity, frustum, camX, camY, camZ) || this.minecraft.player != null && entity.hasIndirectPassenger((Entity)this.minecraft.player))) {
            return false;
        }
    ...
    private void extractBlockOutline(Camera camera, LevelRenderState levelRenderState) {
        ...
        if (!state.isAir() && this.level.getWorldBorder().isWithinBounds(pos)) {
            ...
            CollisionContext context = CollisionContext.of((Entity)camera.entity());
            VoxelShape shape = state.getShape((BlockGetter)this.level, pos, context);
            if (SharedConstants.DEBUG_SHAPES) {
                VoxelShape collisionShape = state.getCollisionShape((BlockGetter)this.level, pos, context);
                VoxelShape occlusionShape = state.getOcclusionShape();
                VoxelShape interactionShape = state.getInteractionShape((BlockGetter)this.level, pos);
    ...
    public void allChanged() {
        ...
        this.sectionUpdateTracker = new SectionUpdateTracker((LevelHeightAccessor)this.level, this.lastViewDistance);
        ...
        for (int z = pos.getZ() - 1; z <= pos.getZ() + 1; ++z) {
            for (int x = pos.getX() - 1; x <= pos.getX() + 1; ++x) {
                for (int y = pos.getY() - 1; y <= pos.getY() + 1; ++y) {
                    this.setSectionDirty(SectionPos.blockToSectionCoord((int)x), SectionPos.blockToSectionCoord((int)y), SectionPos.blockToSectionCoord((int)z), playerChanged);
                }
            }
        }
    }
        ...
        for (int z = z0 - 1; z <= z1 + 1; ++z) {
            for (int x = x0 - 1; x <= x1 + 1; ++x) {
                for (int y = y0 - 1; y <= y1 + 1; ++y) {
                    this.setSectionDirty(SectionPos.blockToSectionCoord((int)x), SectionPos.blockToSectionCoord((int)y), SectionPos.blockToSectionCoord((int)z));
                }
            }
        }
    }
    ...
    public Gizmos.TemporaryCollection collectPerFrameMainThreadGizmos() {
        return Gizmos.withCollector((GizmoCollector)this.mainThreadGizmos);
    }
