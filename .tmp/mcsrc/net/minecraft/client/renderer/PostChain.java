...
implements AutoCloseable {
    public static final Identifier MAIN_TARGET_ID = Identifier.withDefaultNamespace((String)"main");
        ...
        block9: while (true) {
            ...
            switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{PostChainConfig.TextureInput.class, PostChainConfig.TargetInput.class}, (PostChainConfig.Input)input, n)) {
                case 0: {
                    ...
                    String samplerName = string = textureInput.samplerName();
                    String location = string = textureInput.location();
                ...
                case 1: {
                    ...
                    try {
                        String string;
                        samplerName = string = targetInput.samplerName();
                        targetId = string = targetInput.targetId();
    ...
    public void addToFrame(FrameGraphBuilder frame, int screenWidth, int screenHeight, TargetBundle providedTargets) {
        ...
        for (Map.Entry entry : this.internalTargets.entrySet()) {
            ...
            RenderTargetDescriptor descriptor = new RenderTargetDescriptor(target.width().orElse(screenWidth), target.height().orElse(screenHeight), true, (Vector4fc)ARGB.vector4fFromARGB32((int)target.clearColor()), GpuFormat.RGBA8_UNORM);
        ...
        public static TargetBundle of(final Identifier targetId, final ResourceHandle<RenderTarget> target) {
            return new TargetBundle(){
                ...
                public @Nullable ResourceHandle<RenderTarget> get(Identifier id) {
                    return id.equals((Object)targetId) ? this.handle : null;
                }
            };
        }
