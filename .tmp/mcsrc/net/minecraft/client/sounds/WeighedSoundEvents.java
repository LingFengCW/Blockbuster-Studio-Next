...
implements Weighted<Sound> {
    ...
    public WeighedSoundEvents(Identifier location, @Nullable String subtitle) {
        if (SharedConstants.DEBUG_SUBTITLES) {
            MutableComponent components = Component.literal((String)location.getPath());
            if ("FOR THE DEBUG!".equals(subtitle)) {
                components = components.append((Component)Component.literal((String)" missing").withStyle(ChatFormatting.RED));
            }
            this.subtitle = components;
        } else {
            this.subtitle = subtitle == null ? null : Component.translatable((String)subtitle);
        }
    }
