...
public class SoundEngine {
    ...
    private final ChannelAccess channelAccess = new ChannelAccess(this.library, (Executor)((Object)this.executor));
    ...
    private final Object2FloatMap<SoundSource> gainBySource = (Object2FloatMap)Util.make((Object)new Object2FloatOpenHashMap(), map -> map.defaultReturnValue(1.0f));
    ...
    public void reload() {
        ...
        for (SoundEvent sound : BuiltInRegistries.SOUND_EVENT) {
            ...
            LOGGER.warn("Missing sound for event: {}", (Object)BuiltInRegistries.SOUND_EVENT.getKey((Object)sound));
    ...
    public void updateCategoryVolume(SoundSource source, float gain) {
        this.gainBySource.put((Object)source, Mth.clamp((float)gain, (float)0.0f, (float)1.0f));
    ...
    private float calculatePitch(SoundInstance instance) {
        return Mth.clamp((float)instance.getPitch(), (float)0.5f, (float)2.0f);
    }
    ...
    private float calculateVolume(float volume, SoundSource source) {
        return Mth.clamp((float)volume, (float)0.0f, (float)1.0f) * Mth.clamp((float)this.options.getFinalSoundSourceVolume(source), (float)0.0f, (float)1.0f) * this.gainBySource.getFloat((Object)source);
    }
    ...
    public void pauseAllExcept(SoundSource ... ignoredSources) {
        ...
        for (Map.Entry<SoundInstance, ChannelAccess.ChannelHandle> instance : this.instanceToChannel.entrySet()) {
            if (List.of(ignoredSources).contains(instance.getKey().getSource())) continue;
    ...
    public void stop(@Nullable Identifier sound, @Nullable SoundSource source) {
        if (source != null) {
            for (SoundInstance instance : this.instanceBySource.get((Object)source)) {
                if (sound != null && !instance.getIdentifier().equals((Object)sound)) continue;
                this.stop(instance);
            }
        } else if (sound == null) {
            this.stopAll();
            ...
            for (SoundInstance instance : this.instanceToChannel.keySet()) {
                if (!instance.getIdentifier().equals((Object)sound)) continue;
                this.stop(instance);
            }
        }
    }
