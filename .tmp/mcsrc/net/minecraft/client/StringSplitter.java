...
public class StringSplitter {
    ...
    public float stringWidth(@Nullable String str) {
        ...
        StringDecomposer.iterateFormatted((String)str, (Style)Style.EMPTY, (position, style, codepoint) -> {
    ...
    public float stringWidth(FormattedText text) {
        MutableFloat result = new MutableFloat();
        StringDecomposer.iterateFormatted((FormattedText)text, (Style)Style.EMPTY, (position, style, codepoint) -> {
            result.add(this.widthProvider.getWidth(codepoint, style));
            return true;
        });
        return result.floatValue();
    ...
    public int plainIndexAtWidth(String str, int maxWidth, Style style) {
        ...
        StringDecomposer.iterate((String)str, (Style)style, (FormattedCharSink)output);
    ...
    public String plainTailByWidth(String str, int maxWidth, Style style) {
        ...
        StringDecomposer.iterateBackwards((String)str, (Style)style, (position, s, codepoint) -> {
    ...
    public FormattedText headByWidth(FormattedText text, int width, Style initialStyle) {
        ...
        return text.visit((FormattedText.StyledContentConsumer)new FormattedText.StyledContentConsumer<FormattedText>(){
            ...
            public Optional<FormattedText> accept(Style style, String contents) {
                ...
                if (!StringDecomposer.iterateFormatted((String)contents, (Style)style, (FormattedCharSink)output)) {
                    String partial = contents.substring(0, output.getPosition());
                    if (!partial.isEmpty()) {
                        this.collector.append(FormattedText.of((String)partial, (Style)style));
                    }
                    return Optional.of(this.collector.getResultOrEmpty());
                }
                if (!contents.isEmpty()) {
                    this.collector.append(FormattedText.of((String)contents, (Style)style));
                }
    ...
    public int findLineBreak(String input, int max, Style initialStyle) {
        ...
        StringDecomposer.iterateFormatted((String)input, (Style)initialStyle, (FormattedCharSink)finder);
    ...
    public void splitLines(String input, int maxWidth, Style initialStyle, boolean includeAll, LinePosConsumer output) {
        ...
        while (start < size) {
            ...
            boolean endOfText = StringDecomposer.iterateFormatted((String)input, (int)start, (Style)workStyle, (Style)initialStyle, (FormattedCharSink)finder);
            if (endOfText) {
    ...
    public List<FormattedText> splitLines(String input, int maxWidth, Style initialStyle) {
        ...
        this.splitLines(input, maxWidth, initialStyle, false, (style, start, end) -> result.add(FormattedText.of((String)input.substring(start, end), (Style)style)));
        return result;
    }
    ...
    public void splitLines(FormattedText input, int maxWidth, Style initialStyle, BiConsumer<FormattedText, Boolean> output) {
        ...
        block0: while (shouldRestart) {
            ...
            for (LineComponent part : parts.parts) {
                boolean endOfText = StringDecomposer.iterateFormatted((String)part.contents, (int)0, (Style)part.style, (Style)initialStyle, (FormattedCharSink)finder);
                if (!endOfText) {
                    int lineBreak = finder.getSplitPosition();
                ...
                if (!inSkip) {
                    if (position > contentsSize) {
                        ...
                        if (!beforeSplit.isEmpty()) {
                            result.append(FormattedText.of((String)beforeSplit, (Style)element.style));
                        }
                        position += skipSize;
                        inSkip = true;
                    }
