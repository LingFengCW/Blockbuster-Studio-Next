...
public record AnimationFrame(int index, Optional<Integer> time) {
    ...
    public static final Codec<AnimationFrame> CODEC = Codec.either((Codec)ExtraCodecs.NON_NEGATIVE_INT, FULL_CODEC).xmap(either -> (AnimationFrame)either.map(AnimationFrame::new, v -> v), frame -> frame.time.isPresent() ? Either.right((Object)frame) : Either.left((Object)frame.index));
