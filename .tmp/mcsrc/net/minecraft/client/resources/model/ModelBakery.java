...
public class ModelBakery {
    ...
    public static final List<Identifier> DESTROY_STAGES = IntStream.range(0, 10).mapToObj(i -> Identifier.withDefaultNamespace((String)("block/destroy_stage_" + i))).collect(Collectors.toList());
    ...
    public CompletableFuture<BakingResult> bakeModels(MaterialBaker materials, Executor taskExecutor) {
        ...
        CompletableFuture bakedBlockStateModelFuture = ParallelMapTransform.schedule(this.unbakedBlockStateModels, (blockState, model) -> {
            try {
        ...
        CompletableFuture bakedItemStackModelFuture = ParallelMapTransform.schedule(this.clientInfos, (location, clientInfo) -> {
            try {
        ...
        return bakedBlockStateModelFuture.thenCombine((CompletionStage)bakedItemStackModelFuture, (bakedBlockStateModels, bakedItemStateModels) -> new BakingResult(missingModels, (Map<BlockState, BlockStateModel>)bakedBlockStateModels, (Map<Identifier, ItemModel>)bakedItemStateModels, itemStackModelProperties));
    }
