 ...
 *  net.minecraft.server.packs.resources.PreparableReloadListener$StateKey
...
SpriteGetter {
    ...
    public void prepareSharedState(PreparableReloadListener.SharedState currentReload) {
        ...
        currentReload.set(PENDING_STITCH, (Object)new PendingStitchResults(pendingStitches, pendingStitchById, allReadyForUploads));
    }
    ...
    public CompletableFuture<Void> reload(PreparableReloadListener.SharedState currentReload, Executor taskExecutor, PreparableReloadListener.PreparationBarrier preparationBarrier, Executor reloadExecutor) {
        ...
        return ((CompletableFuture)pendingStitches.allReadyToUpload.thenCompose(arg_0 -> ((PreparableReloadListener.PreparationBarrier)preparationBarrier).wait(arg_0))).thenAcceptAsync(unused -> this.updateSpriteMaps(pendingStitches), reloadExecutor);
    }
    ...
    private void updateSpriteMaps(PendingStitchResults pendingStitches) {
        ...
        this.spriteLookup.forEach((? super K id, ? super V sprite) -> {
            ...
            if (!id.texture().equals((Object)MissingTextureAtlasSprite.getLocation()) && (previous = globalSpriteLookup.putIfAbsent(id.texture(), sprite)) != null) {
