...
extends AbstractTickableSoundInstance {
    ...
    public void tick() {
        ...
        if (speed >= 0.01f && this.minecart.level().tickRateManager().runsNormally() && !offRail) {
            this.pitch = Mth.clamp((float)(this.pitch + 0.0025f), (float)0.0f, (float)1.0f);
            this.volume = Mth.lerp((float)Mth.clamp((float)speed, (float)0.0f, (float)0.5f), (float)0.0f, (float)0.7f);
        } else {
            this.pitch = 0.0f;
            this.volume = 0.0f;
        }
