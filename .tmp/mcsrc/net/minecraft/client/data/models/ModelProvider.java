...
implements DataProvider {
    ...
    implements ItemModelOutput {
        ...
        public void finalizeAndValidate() {
            BuiltInRegistries.ITEM.forEach(item -> {
                ...
                if (item instanceof BlockItem && !this.itemInfos.containsKey(blockItem = (BlockItem)item)) {
                    ...
                    this.accept((Item)blockItem, ItemModelUtils.plainModel(targetModel));
                }
            });
        ...
        public CompletableFuture<?> save(CachedOutput cache, PackOutput.PathProvider pathProvider) {
            return DataProvider.saveAll((CachedOutput)cache, ClientItem.CODEC, item -> pathProvider.json(item.builtInRegistryHolder().key().identifier()), this.itemInfos);
        }
    }
    ...
    implements Consumer<BlockModelDefinitionGenerator> {
        ...
        public CompletableFuture<?> save(CachedOutput cache, PackOutput.PathProvider pathProvider) {
            ...
            return DataProvider.saveAll((CachedOutput)cache, BlockStateModelDispatcher.CODEC, pathGetter, (Map)definitions);
        }
    }
    ...
    implements BiConsumer<Identifier, ModelInstance> {
        ...
        public CompletableFuture<?> save(CachedOutput cache, PackOutput.PathProvider pathProvider) {
            return DataProvider.saveAll((CachedOutput)cache, Supplier::get, arg_0 -> ((PackOutput.PathProvider)pathProvider).json(arg_0), this.models);
        }
    }
}
