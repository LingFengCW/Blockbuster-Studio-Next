...
public class ConditionBuilder {
    ...
    public final <T extends Comparable<T>> ConditionBuilder term(Property<T> property, T value, T ... values) {
        List<KeyValueCondition.Term> terms = Stream.concat(Stream.of(value), Stream.of(values)).map(arg_0 -> property.getName(arg_0)).sorted().distinct().map(v -> new KeyValueCondition.Term((String)v, false)).toList();
