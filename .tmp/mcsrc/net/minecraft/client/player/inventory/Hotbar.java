...
public class Hotbar {
    ...
    public static final Codec<Hotbar> CODEC = Codec.PASSTHROUGH.listOf().validate(list -> Util.fixedSize((List)list, (int)SIZE)).xmap(Hotbar::new, hotbar -> hotbar.items);
    ...
    public List<ItemStack> load(HolderLookup.Provider registries) {
        return this.items.stream().map(dynamic -> ItemStack.OPTIONAL_CODEC.parse(RegistryOps.injectRegistryContext((Dynamic)dynamic, (HolderLookup.Provider)registries)).resultOrPartial(error -> LOGGER.warn("Could not parse hotbar item: {}", error)).orElse(ItemStack.EMPTY)).toList();
    }

    public void storeFrom(Inventory inventory, RegistryAccess lookupProvider) {
        RegistryOps registryOps = lookupProvider.createSerializationContext(DEFAULT_OPS);
        ImmutableList.Builder newItems = ImmutableList.builderWithExpectedSize((int)SIZE);
        for (int i = 0; i < SIZE; ++i) {
            ItemStack item = inventory.getItem(i);
            Optional<Dynamic> result = ItemStack.OPTIONAL_CODEC.encodeStart((DynamicOps)registryOps, (Object)item).resultOrPartial(error -> LOGGER.warn("Could not encode hotbar item: {}", error)).map(tag -> new Dynamic(DEFAULT_OPS, tag));
