...
public class GameNarrator {
    ...
    public void updateNarratorStatus(NarratorStatus status) {
        this.clear();
        this.narrateMessage(Component.translatable((String)"options.narrator").append(" : ").append(status.getName()).getString(), true);
        ToastManager toastManager = Minecraft.getInstance().gui.toastManager();
        if (this.narrator.active()) {
            if (status == NarratorStatus.OFF) {
                SystemToast.addOrUpdate(toastManager, SystemToast.SystemToastId.NARRATOR_TOGGLE, (Component)Component.translatable((String)"narrator.toast.disabled"), null);
            } else {
                SystemToast.addOrUpdate(toastManager, SystemToast.SystemToastId.NARRATOR_TOGGLE, (Component)Component.translatable((String)"narrator.toast.enabled"), status.getName());
            }
        } else {
            SystemToast.addOrUpdate(toastManager, SystemToast.SystemToastId.NARRATOR_TOGGLE, (Component)Component.translatable((String)"narrator.toast.disabled"), (Component)Component.translatable((String)"options.narrator.notavailable"));
        }
    }
