...
public class BlockTintCache {
        ...
        try {
            ...
            for (int offsetX = -1; offsetX <= 1; ++offsetX) {
                for (int offsetZ = -1; offsetZ <= 1; ++offsetZ) {
                    long key = ChunkPos.pack((int)(chunkX + offsetX), (int)(chunkZ + offsetZ));
    ...
    private CacheData findOrCreateChunkCache(int x, int z) {
        long key = ChunkPos.pack((int)x, (int)z);
    ...
    private static class CacheData {
        ...
        private final ReentrantReadWriteLock lock = new ReentrantReadWriteLock();
        private static final int BLOCKS_PER_LAYER = Mth.square((int)16);
