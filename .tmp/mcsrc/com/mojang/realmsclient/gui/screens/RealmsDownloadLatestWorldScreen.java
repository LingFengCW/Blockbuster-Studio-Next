...
extends RealmsScreen {
    ...
    private volatile Component status = Component.translatable((String)"mco.download.preparing");
    ...
    public RealmsDownloadLatestWorldScreen(Screen lastScreen, WorldDownload worldDownload, String worldName, BooleanConsumer callback) {
        ...
        this.downloadTitle = Component.translatable((String)"mco.download.title");
    ...
    private void checkDownloadSize() {
        ...
        if (this.getContentLength(this.worldDownload.downloadLink()) >= 0x140000000L) {
            MutableComponent popupMessage = Component.translatable((String)"mco.download.confirmation.oversized", (Object[])new Object[]{Unit.humanReadable(0x140000000L)});
            this.minecraft.gui.setScreen(RealmsPopups.warningAcknowledgePopupScreen(this, (Component)popupMessage, popupScreen -> {
    ...
    private Component createProgressNarrationMessage() {
        ...
        if (this.progress != null) {
            elements.add(Component.translatable((String)"mco.download.percent", (Object[])new Object[]{this.progress}));
            elements.add(Component.translatable((String)"mco.download.speed.narration", (Object[])new Object[]{Unit.humanReadable(this.bytesPersSecond)}));
        }
    ...
    private void extractProgressBar(GuiGraphicsExtractor graphics) {
        ...
        graphics.centeredText(this.font, (Component)Component.translatable((String)"mco.download.percent", (Object[])new Object[]{this.progress}), this.width / 2, 84, -1);
    }
    ...
    private void extractDownloadSpeed0(GuiGraphicsExtractor graphics, long bytesPerSecond) {
        if (bytesPerSecond > 0L) {
            ...
            graphics.text(this.font, (Component)Component.translatable((String)"mco.download.speed", (Object[])new Object[]{Unit.humanReadable(bytesPerSecond)}), this.width / 2 + progressLength / 2 + 15, 84, -1);
        }
    }
        ...
        new Thread(() -> {
            try {
                if (!DOWNLOAD_LOCK.tryLock(1L, TimeUnit.SECONDS)) {
                    this.status = Component.translatable((String)"mco.download.failed");
                    return;
                }
                if (this.cancelled) {
                ...
                this.status = Component.translatable((String)"mco.download.downloading", (Object[])new Object[]{this.worldName});
                ...
                while (!fileDownload.isFinished()) {
                    if (fileDownload.isError()) {
                        fileDownload.cancel();
                        this.errorMessage = Component.translatable((String)"mco.download.failed");
                        this.cancelButton.setMessage(CommonComponents.GUI_DONE);
                        return;
                    }
                    if (fileDownload.isExtracting()) {
                        if (!this.extracting) {
                            this.status = Component.translatable((String)"mco.download.extracting");
                        }
                ...
                this.finished = true;
                this.status = Component.translatable((String)"mco.download.done");
                this.cancelButton.setMessage(CommonComponents.GUI_DONE);
            }
            catch (InterruptedException ignored) {
    ...
    private void downloadCancelled() {
        this.status = Component.translatable((String)"mco.download.cancelled");
    }
