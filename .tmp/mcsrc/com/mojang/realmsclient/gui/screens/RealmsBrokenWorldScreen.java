...
extends RealmsScreen {
    ...
    private final Component[] message = new Component[]{Component.translatable((String)"mco.brokenworld.message.line1"), Component.translatable((String)"mco.brokenworld.message.line2")};
    private int leftX;
    private final List<Integer> slotsThatHasBeenDownloaded = Lists.newArrayList();
    private int animTick;

    public RealmsBrokenWorldScreen(Screen lastScreen, long serverId, boolean isMinigame) {
        super((Component)(isMinigame ? Component.translatable((String)"mco.brokenworld.minigame.title") : Component.translatable((String)"mco.brokenworld.title")));
    ...
    public Component getNarrationMessage() {
        return ComponentUtils.formatList((Collection)Stream.concat(Stream.of(this.title), Stream.of(this.message)).collect(Collectors.toList()), (Component)CommonComponents.SPACE);
    }
    ...
    private void extractSlotFrame(GuiGraphicsExtractor graphics, int x, int y, int xm, int ym, boolean active, String text, int i, long imageId, @Nullable String image, boolean empty) {
        ...
        if (active) {
            ...
            graphics.blit(RenderPipelines.GUI_TEXTURED, texture, x + 3, y + 3, 0.0f, 0.0f, 74, 74, 74, 74, 74, 74, ARGB.colorFromFloat((float)1.0f, (float)c, (float)c, (float)c));
            graphics.blitSprite(RenderPipelines.GUI_TEXTURED, SLOT_FRAME_SPRITE, x, y, 80, 80);
        } else {
            int color = ARGB.colorFromFloat((float)1.0f, (float)0.56f, (float)0.56f, (float)0.56f);
