...
extends RealmsScreen {
    private static final Component POPUP_TEXT = Component.translatable((String)"mco.selectServer.popup");
    private static final Component CLOSE_TEXT = Component.translatable((String)"mco.selectServer.close");
    private static final Identifier BACKGROUND_SPRITE = Identifier.withDefaultNamespace((String)"popup/background");
    private static final Identifier TRIAL_AVAILABLE_SPRITE = Identifier.withDefaultNamespace((String)"icon/trial_available");
    private static final WidgetSprites CROSS_BUTTON_SPRITES = new WidgetSprites(Identifier.withDefaultNamespace((String)"widget/cross_button"), Identifier.withDefaultNamespace((String)"widget/cross_button_highlighted"));
    ...
    public static void updateCarouselImages(ResourceManager resourceManager) {
        Set candidates = resourceManager.listResources("textures/gui/images", s -> s.getPath().endsWith(".png")).keySet();
    ...
    protected void init() {
        ...
        if (this.trialAvailable) {
            this.createTrialButton = this.addRenderableWidget(Button.builder((Component)Component.translatable((String)"mco.selectServer.trial"), ConfirmLinkScreen.confirmLink((Screen)this, CommonLinks.START_REALMS_TRIAL)).bounds(this.right() - 10 - 99, this.bottom() - 10 - 4 - 40, 99, 20).build());
        }
        this.addRenderableWidget(Button.builder((Component)Component.translatable((String)"mco.selectServer.buy"), ConfirmLinkScreen.confirmLink((Screen)this, CommonLinks.BUY_REALMS)).bounds(this.right() - 10 - 99, this.bottom() - 10 - 20, 99, 20).build());
