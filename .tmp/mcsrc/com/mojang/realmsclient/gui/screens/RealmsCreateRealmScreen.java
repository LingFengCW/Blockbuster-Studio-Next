...
extends RealmsScreen {
    private static final Component CREATE_REALM_TEXT = Component.translatable((String)"mco.selectServer.create");
    ...
    public void init() {
        ...
        this.nameBox.setResponder(value -> {
            createButton.active = !StringUtil.isBlank((String)value);
        });
    ...
    private void createWorld(RealmsServer server, boolean initializeSnapshotRealm) {
        if (!server.isSnapshotRealm() && initializeSnapshotRealm) {
            ...
            }, (Component)Component.translatable((String)"mco.upload.preparing"), (Component)Component.empty()));
            ((CompletableFuture)CompletableFuture.supplyAsync(() -> RealmsCreateRealmScreen.createSnapshotRealm(server), (Executor)Util.backgroundExecutor()).thenAcceptAsync(snapshotServer -> {
                if (!canceled.get()) {
                    this.showResetWorldScreen((RealmsServer)snapshotServer);
                }
            }, (Executor)((Object)this.minecraft))).exceptionallyAsync(ex -> {
                ...
                if (patt0$temp instanceof RealmsServiceException) {
                    ...
                    errorMessage = Component.translatable((String)"mco.errorMessage.initialize.failed");
                }
                this.minecraft.gui.setScreen(new RealmsGenericErrorScreen((Component)errorMessage, (Screen)this.lastScreen));
                return null;
