        ...
        try {
            for (Map.Entry entry : this.stagedAllocations.entrySet()) {
                ...
                try {
                    ...
                    Zone zone = this.nodes.iterator();
                    while (zone.hasNext() && (allocation = ((TlsfAllocator)(node = zone.next()).getFirst()).allocate(allocationSize, this.alignSize)) == null) {
                    }
                    if (allocation == null) {
                        zone = Profiler.get().zone("createNewHeap");
                        try {
