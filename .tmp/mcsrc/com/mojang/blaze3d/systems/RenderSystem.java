...
public class RenderSystem {
    ...
    public static void queueFencedTask(Runnable task) {
        PENDING_FENCES.addLast((Object)new GpuAsyncTask(task, RenderSystem.getDevice().createCommandEncoder().createFence()));
    }

    public static void executePendingTasks() {
        GpuAsyncTask task = (GpuAsyncTask)PENDING_FENCES.peekFirst();
        while (task != null) {
            if (task.fence.awaitCompletion(0L)) {
                ...
                task = (GpuAsyncTask)PENDING_FENCES.peekFirst();
                continue;
            }
            return;
        }
    ...
    implements AutoCloseable {
        ...
        private void ensureStorage(int indexCount) {
            ...
            indexCount = Mth.roundToward((int)(indexCount * 2), (int)this.indexStride);
            ...
            int bufferSize = Mth.roundToward((int)(indexCount * type.bytes), (int)4);
