...
implements AutoCloseable {
    ...
    public static NativeImage read(@Nullable Format format, ByteBuffer bytes) throws IOException {
        ...
        PngInfo.validateHeader((ByteBuffer)bytes);
        try (MemoryStack stack = MemoryStack.stackPush();){
    ...
    public int getPixel(int x, int y) {
        return ARGB.fromABGR((int)this.getPixelABGR(x, y));
    }
    ...
    public void setPixel(int x, int y, int pixel) {
        this.setPixelABGR(x, y, ARGB.toABGR((int)pixel));
    }
    ...
    public NativeImage mappedCopy(IntUnaryOperator function) {
        ...
        for (int i = 0; i < pixelCount; ++i) {
            int pixel = ARGB.fromABGR((int)sourceBuffer.get(i));
            int modified = function.applyAsInt(pixel);
            targetBuffer.put(i, ARGB.toABGR((int)modified));
        }
        return result;
    }
    ...
    public int[] getPixels() {
        ...
        for (int i = 0; i < result.length; ++i) {
            result[i] = ARGB.fromABGR((int)result[i]);
        }
        return result;
    }
    ...
    public Transparency computeTransparency(int x0, int y0, int x1, int y1) {
        ...
        for (int y = y0; y < y1; ++y) {
            for (int x = x0; x < x1; ++x) {
                int alpha = ARGB.alpha((int)buffer.get(x + y * this.width));
                if (alpha == 0) {
